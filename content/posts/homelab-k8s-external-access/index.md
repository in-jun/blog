---
title: "홈랩 쿠버네티스 #5 - Cloudflare DDNS로 외부 접근 설정하기"
...
[이전 글](/posts/homelab-k8s-internal-services/)에서는 Traefik 인그레스 컨트롤러를 설치하고 내부와 외부 서비스를 분리하여 관리 인터페이스를 안전하게 접근할 수 있도록 구성했다. 이번 글에서는 홈랩 쿠버네티스 클러스터의 서비스를 외부 인터넷에서 접근할 수 있도록 DNS 설정, 동적 IP 관리, 라우터 포트포워딩을 구성하는 방법을 알아본다.
...
이번 글에서는 홈랩 쿠버네티스 클러스터의 서비스를 외부 인터넷에서 접근할 수 있도록 Cloudflare DNS 설정, Cloudflare Workers를 활용한 커스텀 DDNS 구현, 라우터 포트포워딩 구성 방법을 살펴보았다. 내부용과 외부용 로드밸런서를 분리하고 포트포워딩 대상을 외부용 IP로만 설정함으로써 관리 인터페이스가 외부에 노출되는 것을 방지할 수 있다.

다음 글에서는 HashiCorp Vault를 설치하여 비밀번호, API 키 같은 민감한 정보를 안전하게 관리하는 방법을 알아본다.

[다음 글: 홈랩 쿠버네티스 #6 - Vault와 ArgoCD로 시크릿 관리하기](/posts/homelab-k8s-secrets/)
