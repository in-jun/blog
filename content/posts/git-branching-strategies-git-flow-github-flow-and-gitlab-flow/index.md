---
title: "Git 브랜치 전략, Git Flow와 GitHub Flow 그리고 GitLab Flow"
...
단점으로는 브랜치가 많아 복잡도가 높고 학습 곡선이 가파르며, 병합이 빈번해 충돌 해결에 시간이 소요된다는 점이 있다. 또한 지속적 배포(CD) 환경에는 부적합하며, 2020년 Vincent Driessen도 "웹 앱에는 GitHub Flow가 더 적합할 수 있다"고 언급했다.
...
GitLab Flow는 하나의 고정된 구조라기보다 main 브랜치를 중심으로 상황에 맞게 운영하는 방식에 가깝고, 대표적으로 두 가지 변형이 자주 소개된다.
...
위 그림은 GitLab 공식 문서에서 소개하는 환경 브랜치 방식에 가까운 예시로, 기능 개발과 main 브랜치 통합은 계속 진행하면서 배포 후보를 test와 production 같은 환경 브랜치로 흘려보내는 흐름을 단순화해 보여준다.
...
Git 브랜치 전략은 2010년 Vincent Driessen의 Git Flow를 시작으로 GitHub Flow(2011), GitLab Flow(2014)로 발전해왔다. 각 전략은 릴리스 주기, 프로젝트 규모, 배포 환경에 따라 적합한 상황이 다르다. Git Flow는 체계적인 릴리스 관리에, GitHub Flow는 지속적 배포에, GitLab Flow는 환경별 배포 관리에 강점이 있다. 결국 중요한 것은 팀 전체가 이해하고 일관되게 따를 수 있는 전략을 선택한 뒤, 필요에 따라 유연하게 조정하는 일이다.
