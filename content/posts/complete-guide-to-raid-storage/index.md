...
RAID(Redundant Array of Independent Disks)는 여러 개의 물리적 하드 디스크를 하나의 논리적 단위로 묶어 데이터 안정성을 높이거나 입출력 성능을 개선하는 저장 기술이다. 이 개념은 1988년 캘리포니아 대학교 버클리 캠퍼스의 David Patterson, Garth A. Gibson, Randy Katz가 발표한 논문 "A Case for Redundant Arrays of Inexpensive Disks"에서 처음 제안됐다. 당시 RAID의 약자는 "Inexpensive Disks(저렴한 디스크)"를 뜻했지만, 현재는 "Independent Disks(독립적인 디스크)"라는 표현이 더 널리 쓰인다. 핵심 목적은 단일 대용량 디스크 대신 여러 개의 소형 디스크를 조합해 비용 효율성과 신뢰성을 함께 확보하는 데 있었다.

RAID의 기본 아이디어는 데이터를 여러 디스크에 분산 저장하는 스트라이핑(Striping), 동일하게 복제하는 미러링(Mirroring), 오류 검출과 복구를 위한 패리티(Parity)를 조합해 단일 디스크보다 더 나은 성능이나 신뢰성을 제공하는 것이다. 이 방식은 현대 서버 시스템, 엔터프라이즈 스토리지, NAS(Network Attached Storage), SAN(Storage Area Network) 등 거의 모든 데이터 중심 인프라에서 핵심 구성 요소로 자리잡았고, 클라우드 컴퓨팅 환경의 물리적 스토리지 계층에서도 널리 활용된다.

## RAID의 역사적 배경과 발전 과정

1980년대 후반, 메인프레임과 미니컴퓨터에서 사용되던 대용량 디스크는 매우 고가였다. 또한 디스크가 고장나면 전체 시스템이 멈추는 단일 장애점(Single Point of Failure) 문제가 심각했다. 이를 해결하기 위해 버클리 대학 연구진은 여러 개의 저렴한 디스크를 병렬로 연결해 단일 대용량 디스크와 동등하거나 더 나은 성능과 신뢰성을 얻을 수 있다는 개념을 제시했다. 초기 RAID 논문은 RAID 레벨 1부터 5까지를 정의했고, 이후 RAID 0(스트라이핑), RAID 6(이중 패리티), RAID 10(미러링과 스트라이핑 조합) 등이 추가되며 현재의 표준 구성이 자리잡았다.

1990년대에는 하드웨어 RAID 컨트롤러가 상용화되면서 엔터프라이즈 시장에서 빠르게 채택됐다. Dell PERC(PowerEdge RAID Controller), HP Smart Array, IBM ServeRAID 등 주요 서버 제조사도 자체 RAID 솔루션을 제공하기 시작했다. 2000년대에는 리눅스 커널의 md(Multiple Device) 드라이버와 ZFS, Btrfs 같은 차세대 파일 시스템이 소프트웨어 RAID를 지원하면서 비용 효율적인 구성이 가능해졌다. 최근에는 NVMe SSD 같은 고성능 스토리지 장치의 등장으로 RAID의 역할과 구성 방식도 계속 진화하고 있다.
...
하드웨어 RAID는 전용 RAID 컨트롤러 카드나 메인보드에 내장된 RAID 칩으로 구현한다. RAID 컨트롤러는 독립적인 프로세서와 전용 메모리(캐시), 배터리 백업 유닛(BBU, Battery Backup Unit) 또는 슈퍼캐패시터를 탑재해 호스트 시스템의 CPU 부하 없이 RAID 연산을 처리한다. 운영체제는 RAID 어레이를 단일 물리 디스크로 인식하므로 OS와 비교적 독립적으로 동작하며 부팅 디스크로도 사용할 수 있다. 또한 캐시 메모리를 활용한 쓰기 캐싱(Write-back Cache)으로 쓰기 성능을 높일 수 있고, BBU나 슈퍼캐패시터가 있으면 정전 시에도 캐시 데이터를 보호할 수 있다.
...
소프트웨어 RAID는 운영체제 커널이나 볼륨 관리자가 RAID 기능을 구현하는 방식이다. 별도의 하드웨어 없이 일반 디스크 컨트롤러(SATA, SAS, NVMe)에 연결된 디스크들을 묶어 RAID를 구성할 수 있다. 리눅스에서는 md(Multiple Device) 드라이버와 mdadm 유틸리티를 통해 RAID 0, 1, 4, 5, 6, 10을 지원하며, LVM(Logical Volume Manager)과 결합하면 볼륨 관리 유연성이 높아진다. ZFS(Zettabyte File System)와 Btrfs(B-tree File System) 같은 차세대 파일 시스템은 RAID 기능을 파일 시스템 레벨에 통합해 제공한다.
...
펌웨어 RAID는 메인보드 칩셋에 내장된 RAID 기능을 사용하는 방식이다. BIOS/UEFI 수준에서 설정하지만 실제 RAID 연산은 운영체제 드라이버가 수행하므로 흔히 "Fake RAID" 또는 "Host RAID"라고 부른다. 대표적으로 Intel RST(Rapid Storage Technology), AMD RAIDXpert 등이 있으며, 소비자용 메인보드에서 자주 볼 수 있다. BIOS에서 설정할 수 있어 부팅 디스크 구성이 가능하다는 장점이 있지만, 실제 연산은 CPU를 사용하므로 성능과 안정성은 제한적이고 벤더 종속적인 드라이버가 필요해 이식성도 떨어진다.

일반적으로 펌웨어 RAID는 하드웨어 RAID의 비용 부담이 크고 소프트웨어 RAID 설정이 번거로운 경우에 선택한다. 다만 엔터프라이즈 환경에서는 보통 권장되지 않으며, 가정용 NAS나 워크스테이션에서 간단한 RAID 1(미러링) 구성 정도로 제한적으로 사용된다.
...
RAID 5는 블록 레벨 스트라이핑과 분산 패리티(Distributed Parity)를 결합한 방식이다. 데이터 블록과 함께 XOR 연산으로 생성된 패리티 정보를 모든 디스크에 순환 분산 저장하며, 최소 3개의 디스크가 필요하다. 1개 디스크 분량의 용량을 패리티에 할당하므로 용량 효율은 (N-1)/N이다. 단일 디스크 고장 시에는 나머지 디스크의 데이터와 패리티를 사용해 손실된 데이터를 재구축할 수 있어, RAID 0의 성능과 RAID 1의 안정성을 절충한 균형잡힌 솔루션으로 중소규모 서버와 NAS에서 널리 사용된다.
...
RAID 10은 RAID 1(미러링)과 RAID 0(스트라이핑)을 결합한 중첩 RAID 레벨이다. 먼저 디스크를 2개씩 쌍으로 묶어 RAID 1 미러를 만들고, 이 미러 쌍들을 다시 RAID 0으로 스트라이핑해 높은 성능과 신뢰성을 함께 확보한다. 최소 4개의 디스크가 필요하며 전체 용량의 50%를 미러링에 사용하지만, 읽기와 쓰기 모두 우수한 성능을 제공한다. 또한 각 미러 쌍에서 1개씩의 디스크 고장을 허용하므로 특정 조건에서는 다중 디스크 고장도 견딜 수 있어, 고성능 데이터베이스와 트랜잭션 처리 시스템에서 선호된다.
...
| 비디오 편집/렌더링 | RAID 0 또는 5 | 높은 순차 처리량, 백업 별도 구축 |
...
RAID는 현대 데이터 인프라에서 핵심적인 스토리지 가상화 기술이다. 디스크 하드웨어 고장으로부터 시스템 가용성을 보호하고, 워크로드에 따라 성능도 향상시킬 수 있다. 1988년 버클리 대학의 연구에서 시작된 뒤 30년 이상 발전하며 엔터프라이즈 스토리지의 표준으로 자리잡았고, RAID 0부터 RAID 60까지 다양한 레벨은 성능, 안정성, 비용 효율성의 서로 다른 조합을 제공한다.

적절한 RAID 레벨을 선택할 때는 워크로드 특성(랜덤 vs 순차, 읽기 vs 쓰기), 데이터 중요도(복구 가능성, 다운타임 허용도), 성능 요구사항(IOPS, 처리량, 지연시간), 비용 제약(디스크 수, 하드웨어 투자), 용량 요구사항을 함께 고려해야 한다. 일반적으로 운영체제와 중요 데이터베이스에는 RAID 1 또는 RAID 10이, 대용량 파일 서버와 백업 스토리지에는 RAID 5 또는 RAID 6가, 고성능 임시 저장소에는 RAID 0이 적합하다.

RAID를 구성한 뒤에는 정기적인 모니터링과 예방적 유지보수가 필수다. SMART 모니터링으로 디스크 건강 상태를 추적하고, 월간 스크러빙으로 잠재적 오류를 조기에 발견하며, 핫스페어를 준비해 재구축 시간을 최소화하는 것이 좋다. 3-5년 주기의 예방적 디스크 교체도 도움이 된다. 무엇보다 RAID는 백업의 대체재가 아니므로, 3-2-1 백업 규칙(3개 사본, 2개 미디어, 1개 오프사이트)을 포함한 별도의 데이터 보호 전략을 함께 수립해야 한다.

대용량 디스크 환경에서는 재구축 중 추가 고장이나 URE 발생 위험이 커서 RAID 5보다 RAID 6가 더 권장되는 추세다. 한편 NVMe SSD 같은 고성능 스토리지의 등장으로 소프트웨어 RAID의 CPU 오버헤드는 상대적으로 덜 부담스러워졌고, 비용 효율적인 선택지도 늘어났다. 또한 ZFS와 Btrfs 같은 차세대 파일 시스템은 RAID 기능과 함께 데이터 무결성 검증, 스냅샷, 압축 같은 고급 기능을 제공하므로, 새로운 시스템을 구축할 때 함께 검토할 만하다.
