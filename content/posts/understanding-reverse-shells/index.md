...
리버스 셸(Reverse Shell)은 일반적인 원격 접속의 연결 방향을 뒤집는 공격 기법이다. 침해당한 대상 시스템이 공격자의 시스템으로 직접 연결을 시도하고, 공격자는 그 연결을 통해 대상 시스템의 셸에 접근하게 된다. 이 방식은 현대 네트워크 환경에서 흔한 "인바운드 연결 차단, 아웃바운드 연결 허용" 정책을 우회할 수 있기 때문에, 제한된 네트워크 내부의 시스템에 접근하는 데 자주 활용된다.

> **교육적 목적과 윤리적 사용**
>
> 이 글은 정보 보안 전문가, 시스템 관리자, 침투 테스터가 리버스 셸의 작동 원리와 방어 기법을 이해하도록 돕기 위한 교육용 글이다. 모든 기술적 내용은 반드시 명시적인 권한을 부여받은 시스템에서만 테스트해야 하며, 무단으로 타인의 시스템을 침해하거나 공격하는 행위는 관련 법률에 따라 처벌받을 수 있다.

## 리버스 쉘의 작동 원리와 네트워크 구조

리버스 셸의 핵심은 전통적인 클라이언트-서버 모델의 역할을 뒤집는 데 있다. 일반적인 원격 접속에서는 공격자가 클라이언트가 되어 대상 시스템의 서비스 포트(예: SSH 22번, RDP 3389번)로 접속을 시도한다. 반면 리버스 셸에서는 대상 시스템이 클라이언트가 되어 공격자가 운영하는 리스너 서버로 능동적으로 연결을 수립한다. 이 구조 덕분에 아웃바운드 필터링 우회, NAT 환경에서의 접근성 확보, 기본적인 네트워크 모니터링 회피가 쉬워진다.
...
공격자는 수립된 리버스 셸 세션을 통해 대상 시스템에서 임의의 명령어를 실행하고, 파일 시스템 탐색, 권한 상승 시도, 추가 악성코드 다운로드 및 실행, 내부 네트워크 스캔, 민감 데이터 탈취, 로그 삭제 같은 다양한 사후 침투 활동을 수행할 수 있다. 연결이 끊어지더라도 재연결을 시도하는 지속성 메커니즘을 추가해 장기간 접근을 유지하기도 한다.
...
리버스 셸은 다양한 프로그래밍 언어, 스크립팅 환경, 네트워크 도구를 활용해 구현할 수 있다. 어떤 방법을 선택할지는 대상 시스템의 운영체제, 설치된 소프트웨어 스택, 네트워크 제약, 보안 통제 수준에 따라 달라진다. 실제 공격에서는 탐지 회피와 안정성을 고려해 여러 방식이 조합되기도 한다.
...
Python은 강력한 소켓 프로그래밍 API와 크로스 플랫폼 호환성을 제공해 안정적이고 기능이 많은 리버스 셸을 구현하기 좋다. 대부분의 Linux 배포판과 macOS에 기본 설치되어 있고, Windows 환경에서도 자동화 도구로 자주 사용되기 때문에 접근성이 높은 편이다.
...
이 버전은 연결이 끊어지거나 실패하더라도 5분(300초) 간격으로 재연결을 시도해 지속성을 유지한다. 네트워크가 불안정하거나 시스템이 재부팅된 뒤에도 C2 서버와 다시 연결을 시도할 수 있다.
...
이 기법은 보안 관리자가 근무하지 않는 시간대에만 활동해 실시간 탐지와 대응을 어렵게 만든다. 또한 주간 시간대의 네트워크 트래픽에서 이상 징후가 두드러지지 않도록 하는 데 도움이 된다.
...
리버스 셸은 방화벽과 NAT 환경을 우회하는 대표적인 공격 기법 가운데 하나다. 초기 침투에 성공한 공격자가 피해 시스템에 지속적인 원격 접근을 확보하는 수단으로 자주 사용되며, 단순한 Bash 원라이너부터 암호화된 멀티스테이지 페이로드, HTTP/DNS 터널링, 시간 기반 회피 기법까지 다양한 구현 방법이 존재한다.
