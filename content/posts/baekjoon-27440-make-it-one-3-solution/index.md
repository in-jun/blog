...
백준 27440번 "1로 만들기 3"은 10^18 범위의 매우 큰 정수를 세 가지 연산(3으로 나누기, 2로 나누기, 1 빼기)으로 1로 만들 때 필요한 최소 연산 횟수를 구하는 문제다. 입력 범위가 10^18에 달하므로 일반적인 동적 프로그래밍 접근법(O(N) 시간, O(N) 공간)으로는 해결할 수 없다. 대신 재귀적 점화식과 맵 기반 메모이제이션을 활용해 O(log² n) 시간 복잡도로 해결해야 한다.
...
그러나 이 문제에서 N의 범위는 10^18이므로, 이 접근법은 메모리와 시간 측면 모두에서 현실적으로 불가능하다.
...
맵(`std::map` 또는 `std::unordered_map`)을 사용해 이미 계산한 값을 저장하면 중복 계산을 막아 효율성을 크게 높일 수 있다.
...
- `memo`: 이미 계산한 결과를 저장하는 맵으로, 키는 정수 n이고 값은 n을 1로 만드는 최소 연산 횟수이다.
...
각 재귀 호출에서 n은 n/2 또는 n/3으로 줄어드므로, 재귀 호출 깊이는 O(log n)이다.

### 고유한 상태의 수

각 재귀 호출에서 두 개의 하위 문제(n/2와 n/3)가 생성되므로, 겉으로는 상태 수가 빠르게 늘어나는 것처럼 보일 수 있다. 그러나 실제로는 많은 경로가 같은 값으로 수렴하므로 고유한 상태 수는 훨씬 적으며, 대략 O(log² n) 수준으로 볼 수 있다.
...
메모이제이션에 사용되는 맵의 크기가 고유 상태 수에 비례하므로 공간 복잡도는 O(log² n)이다. N = 10^18일 때 log₂(10^18) ≈ 60이므로, 최대 약 3600개의 상태만 저장하면 되어 메모리 사용량이 매우 적다.
...
백준 27440번 "1로 만들기 3"은 큰 입력 범위 때문에 일반적인 O(N) 동적 프로그래밍으로는 해결할 수 없다. 대신 n에서 n/2와 n/3으로 가는 경로를 비교하는 재귀 점화식과 메모이제이션을 활용하면 매우 큰 수에 대해서도 효율적으로 답을 구할 수 있다.
