...
네트워크 스위치는 1990년대 초 이더넷 네트워크의 성능 한계를 보완하기 위해 등장한 장비다. 기존 허브는 모든 포트로 데이터를 브로드캐스트해 비효율이 컸지만, 스위치는 목적지 MAC 주소를 기반으로 프레임을 선택적으로 전달해 이 문제를 개선했다. 이런 특성 덕분에 스위치는 현대 네트워크 인프라의 핵심 구성요소가 되었으며, OSI(Open Systems Interconnection) 7계층 모델에서 어느 계층의 정보를 처리하느냐에 따라 L1부터 L7까지 여러 종류로 나뉜다.
...
1990년 Kalpana사가 최초의 이더넷 스위치인 EtherSwitch를 출시하면서 스위칭 기술의 상용화가 시작되었다. 이 장비는 MAC 주소 테이블을 사용해 프레임을 특정 포트로만 전달함으로써 네트워크 효율성을 크게 높였다. 1994년 Cisco가 Kalpana를 인수한 뒤 스위칭 기술은 빠르게 엔터프라이즈 네트워크의 표준으로 자리 잡았다. 이후 VLAN, 스패닝 트리 프로토콜(STP), QoS 등 다양한 기능이 추가되면서 스위치는 단순한 프레임 전달 장치에서 지능형 네트워크 장비로 발전했다.
...
L2 스위치는 관리 기능의 유무에 따라 비관리형(Unmanaged) 스위치와 관리형(Managed) 스위치로 구분된다. 비관리형 스위치는 별도 설정 없이 연결만 하면 바로 작동하는 플러그 앤 플레이 방식이어서 소규모 네트워크나 가정용 환경에 적합하다. 반면 관리형 스위치는 웹 인터페이스나 CLI(Command Line Interface)를 통해 VLAN, QoS, 포트 보안 등 다양한 설정을 적용할 수 있어 기업 환경에서 주로 사용된다.
...
L3 스위치는 ASIC(Application-Specific Integrated Circuit) 칩을 사용해 라우팅을 하드웨어 수준에서 처리하므로, 일반적인 소프트웨어 중심 라우팅 장비보다 높은 처리 성능을 제공하는 경우가 많다. 이 장비는 서로 다른 VLAN이나 서브넷 간의 통신을 가능하게 하며, OSPF, EIGRP, BGP 등 동적 라우팅 프로토콜을 지원해 대규모 네트워크에서 최적 경로를 자동으로 계산하고 변화에 유연하게 대응할 수 있다. 또한 ACL(Access Control List)을 통한 세밀한 트래픽 필터링과 QoS 정책 적용도 가능하다.
...
L4 스위치는 로드 밸런싱 외에도 NAT(Network Address Translation)를 통해 내부 서버의 실제 IP 주소를 숨기고 가상 IP(VIP)로 서비스를 제공할 수 있다. 또한 세션 지속성(Session Persistence 또는 Sticky Session) 기능을 사용해 특정 클라이언트의 요청을 동일한 서버로 전달함으로써 세션 기반 애플리케이션의 정상 동작을 돕는다. 여기에 SSL 오프로딩(SSL Offloading) 기능을 지원해 암호화와 복호화 작업을 장비에서 처리함으로써 백엔드 서버의 CPU 부하를 줄일 수 있으며, 연결 다중화(Connection Multiplexing)로 서버 연결을 재사용해 연결 설정 오버헤드도 낮출 수 있다.
...
L7 스위치는 패킷의 페이로드(Payload)까지 심층 분석하는 DPI(Deep Packet Inspection) 기술을 사용해, 단순한 IP와 포트 정보를 넘어 요청의 실제 내용을 기준으로 트래픽을 처리할 수 있다. 예를 들어 URL 경로에 따라 서로 다른 서버 그룹으로 트래픽을 라우팅하거나, HTTP 헤더의 특정 값을 기준으로 로드 밸런싱 결정을 내릴 수 있다. 또한 웹 애플리케이션 방화벽(WAF) 기능을 통해 SQL 인젝션, XSS(Cross-Site Scripting), CSRF(Cross-Site Request Forgery) 등 OWASP Top 10에 포함되는 공격을 탐지하고 차단할 수 있다. 더불어 API 게이트웨이 역할을 수행하며 API 요청의 인증, 속도 제한, 변환 등을 처리할 수 있다.
...
네트워크 스위치는 OSI 계층에 따라 L1 허브부터 L7 ADC까지 다양한 형태로 구분되며, 각 장비는 처리하는 정보의 범위에 따라 역할과 강점이 달라진다. 낮은 계층의 스위치는 빠르고 단순한 전달에 강점이 있고, 높은 계층의 스위치는 더 많은 문맥을 바탕으로 정교한 제어와 보안 기능을 제공한다. 따라서 효율적인 네트워크를 구축하려면 규모, 트래픽 특성, 보안 요구사항, 운영 복잡도를 함께 고려해 필요한 계층의 장비를 선택해야 한다.
