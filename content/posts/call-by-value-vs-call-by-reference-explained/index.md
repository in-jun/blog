...
참조 방식의 전달에서는 함수가 별도의 복사본 대신 원본 변수에 도달할 수 있는 수단을 전달받는다. 따라서 함수 내부의 변경이 호출한 쪽의 데이터에 그대로 반영될 수 있다.
...
C 언어는 언어 차원에서 순수한 Call by Reference를 지원하지 않으며, 기본적으로는 값을 복사해 전달한다. 대신 원본 데이터를 수정해야 할 때는 포인터를 사용해 참조 전달과 유사한 효과를 만들 수 있고, 이 방식은 "Call by Address" 또는 "Simulated Call by Reference"라고도 불린다.
