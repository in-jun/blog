...
> **HashiCorp Vault란?**
>
> HashiCorp Vault는 비밀번호, API 키, 인증서 같은 민감한 정보를 중앙에서 안전하게 저장하고 관리하는 시크릿 관리 도구다. 세밀한 접근 제어, 감사 로깅, 자동 시크릿 갱신, 쿠버네티스 연동 같은 기능을 제공한다.

홈랩에서는 GitOps 흐름을 유지하면서 시크릿 암호화, 접근 제어, 갱신 자동화까지 함께 다루기 위해 Vault를 선택했다.

## Vault 설치하기

### 1. GitOps 구성을 위한 디렉토리 준비

내 홈랩에서는 모든 리소스를 GitOps로 관리하고 있어서 Vault도 같은 흐름으로 올렸다. 시작은 필요한 디렉토리 구조를 만드는 것부터였다:
...
`values.yaml`에는 Vault 설정을 아래처럼 넣었다:
...
Vault UI는 이전 글에서 올린 Traefik을 통해 내부망에서만 접근하도록 열었다.
...
`entryPoints`를 `intweb`과 `intwebsec`로 설정해 Vault UI가 내부 네트워크에서만 열리도록 구성했다. 시크릿 관리 인터페이스인 만큼 외부에 노출하지 않는 편이 안전하다고 판단했다.
...
이 커밋 이후에는 ArgoCD가 저장소 변경을 감지해 Vault를 자동으로 배포했다.

## Vault 초기화 및 언실링

Vault 설치 후에는 초기화(initialization)와 언실링(unsealing)이라는 두 가지 중요한 단계가 필요하다. 초기화 과정에서 암호화 키를 생성하고, 언실링을 거쳐 Vault를 사용할 수 있는 상태로 전환한다. 보안을 고려해 이 과정은 자동화하지 않고 수동으로 진행했다.

### 1. 초기화 수행

초기화는 Vault 파드에 직접 들어가서 수동으로 진행했다:
...
초기화가 끝난 뒤에는 기본 설정대로 5개 키 중 3개를 사용해 언실링했다:
...
> **Shamir의 비밀 공유 알고리즘**
>
> Vault는 Shamir의 비밀 공유(Shamir's Secret Sharing) 알고리즘을 사용해 마스터 키를 여러 조각으로 분할한다. 기업 환경에서는 이 5개의 키를 서로 다른 관리자에게 나눠 보관하고, 최소 3개의 키가 있어야 Vault를 열 수 있도록 운영한다. 홈랩에서는 한 사람이 관리하지만 이런 엔터프라이즈 보안 방식을 직접 경험해볼 수 있었다.

## Vault 웹 UI 접근

Vault가 활성화된 뒤에는 웹 UI로도 확인했고, 내 로컬 머신 hosts 파일에는 아래 항목을 추가했다:
...
설치와 초기화가 끝난 뒤에는 쿠버네티스와 연동하기 위한 기본 설정을 이어서 진행했다.

### 1. 쿠버네티스 인증 설정

쿠버네티스 인증은 파드가 서비스 계정 토큰으로 Vault에 인증하도록 구성했다. 실제로는 Vault 파드 안에서 아래 명령을 실행했다:
...
접근 제어는 정책(Policy)부터 잡았다. 애플리케이션용 샘플 정책은 아래처럼 만들었다:
...
이 정책으로 `secret/data/app/*` 경로의 모든 시크릿에 대한 읽기 권한과 메타데이터 조회 권한을 부여했다.

이후 쿠버네티스 인증용 역할도 만들었다:
...
이제 Vault에 기본적인 시크릿을 저장했으니, 다음으로 이 값을 쿠버네티스 애플리케이션에서 활용하는 두 가지 방법을 구현했다.

## Vault Secrets Operator 설치

첫 번째 접근법은 Vault Secrets Operator를 사용하는 방식이었다. Vault의 시크릿을 쿠버네티스 Secret으로 자동 동기화해 기존 애플리케이션 코드를 거의 수정하지 않고 활용할 수 있다는 점이 장점이었다.
...
첫 번째 실험은 Vault Secrets Operator를 통해 Vault 시크릿을 쿠버네티스 Secret으로 동기화하는 방식이었다.
...
`refreshAfter: 30s` 설정을 두면 Vault에서 시크릿이 바뀌었을 때 쿠버네티스 Secret도 약 30초 안에 함께 업데이트된다.
...
두 번째로는 ArgoCD Vault Plugin도 구성해봤다. 이 방식은 Git 저장소에는 시크릿 참조만 남기고, 실제 값은 ArgoCD가 배포 시점에 Vault에서 가져오게 한다.
...
이 설정은 ArgoCD repo-server에 Vault Plugin을 사이드카 컨테이너로 추가해 Helm 차트 렌더링 과정에 플러그인을 연결한다.
...
이제 앞에서 구성한 두 방식을 실제 애플리케이션에 적용해봤다.
...
이번 글에서는 내 홈랩 쿠버네티스 클러스터에 HashiCorp Vault를 올리고, 두 가지 방식으로 시크릿을 연결한 과정을 정리했다. Vault Secrets Operator는 기존 애플리케이션의 Secret 사용 방식을 유지한 채 적용하기 수월했고, ArgoCD Vault Plugin은 GitOps 흐름을 유지하면서 Git에 민감한 값을 남기지 않는 구성이 가능했다.
