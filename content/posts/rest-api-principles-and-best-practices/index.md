...
REST가 등장하기 전 웹 서비스는 주로 SOAP(Simple Object Access Protocol)과 XML-RPC를 사용했다. 이 프로토콜들은 복잡한 XML 기반 메시지 포맷, 엄격한 타입 시스템, WSDL(Web Services Description Language)을 통한 서비스 정의를 요구해 구현과 유지보수가 어려웠다. SOAP은 엔터프라이즈 환경에서 트랜잭션, 보안, 메시지 신뢰성 등을 지원하는 장점이 있었지만, 단순한 데이터 조회에도 복잡한 XML 봉투(envelope)를 사용해야 하는 오버헤드가 있었다.

### Roy Fielding의 기여

Roy Fielding은 HTTP/1.0과 HTTP/1.1 명세의 주요 저자이자 Apache HTTP Server 프로젝트의 공동 창립자였다. 그는 웹 아키텍처를 깊이 이해하고 있었고, 박사 학위 논문에서 웹이 성공할 수 있었던 이유를 분석해 이를 REST라는 아키텍처 스타일로 체계화했다. REST는 기존 웹 인프라(HTTP, URI, 캐시, 프록시)를 최대한 활용하면서도 단순하고 확장 가능한 시스템을 구축할 수 있도록 설계되었다.

### REST API의 확산

REST가 실제로 널리 채택되기 시작한 것은 2000년대 중반 이후였다. Flickr(2004년), Amazon Web Services(2006년), Twitter(2006년) 등이 REST API를 공개하면서 REST는 웹 API의 표준으로 자리 잡았다. JSON 포맷의 부상과 함께 REST는 SOAP의 복잡성을 피하고 싶어 하던 개발자들에게 매력적인 대안이 되었고, Ajax와 모바일 앱의 등장으로 그 인기가 더욱 높아졌다.
...
> 4. **HATEOAS**: 응답에 다음에 수행할 수 있는 작업에 대한 하이퍼링크 포함
...
응답에 관련 리소스의 링크를 포함해 클라이언트가 다음에 수행할 수 있는 작업을 스스로 발견하게 하는 REST의 핵심 원칙이다.
...
REST API 설계는 단순한 기술 선택이 아니라 사용자 경험과 비즈니스 요구사항까지 함께 고려해야 하는 과정이다. REST의 6가지 제약 조건을 이해하고 Richardson 성숙도 모델을 참고하되, 이를 맹목적으로 따르기보다 각 설계 결정의 트레이드오프를 파악해 프로젝트 특성에 맞게 유연하게 적용하는 것이 중요하다. 완벽한 RESTful API(Level 3)를 구현하는 것보다 실용적이고 일관성 있는 REST-like API(Level 2)를 목표로 하면서, 충실한 문서화와 개발자 경험을 함께 챙기는 것이 성공적인 API 설계의 핵심이다.
