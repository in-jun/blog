...
플로이드-워셜(Floyd-Warshall) 알고리즘은 그래프의 모든 정점 쌍(All-Pairs) 사이의 최단 경로를 찾는 동적 프로그래밍 기반 알고리즘이다. 다익스트라나 벨만-포드 알고리즘과 달리 한 번의 실행으로 모든 쌍의 최단 경로를 동시에 계산할 수 있으며, 1962년 Robert Floyd와 Stephen Warshall이 각각 독립적으로 발표했다. 시간 복잡도는 O(V³)이며, 음수 가중치 간선을 처리할 수 있고 음수 사이클의 존재 여부도 탐지할 수 있어 네트워크 지름 계산, 추이적 폐쇄 연산, 데이터베이스 쿼리 최적화 등 다양한 분야에서 활용된다.
...
플로이드-워셜 알고리즘은 1962년 미국의 컴퓨터 과학자 Robert W. Floyd가 Communications of the ACM에 발표한 논문 "Algorithm 97: Shortest Path"에서 최단 경로 문제에 적용한 형태로 제시했다. 같은 해 Stephen Warshall도 Journal of the ACM에 발표한 논문 "A Theorem on Boolean Matrices"에서 추이적 폐쇄(Transitive Closure) 문제에 적용한 형태를 독립적으로 발표했다. 오늘날에는 두 연구자의 이름을 따서 플로이드-워셜 알고리즘이라고 부르지만, 프랑스의 수학자 Bernard Roy가 1959년 연구에서 본질적으로 같은 알고리즘을 이미 기술했기 때문에 일부 유럽 문헌에서는 "Roy-Floyd-Warshall 알고리즘"이라는 이름도 사용한다.
...
V × V 크기의 2차원 배열 D를 생성한다. 인접한 정점 사이는 간선의 가중치로 초기화하고, 연결되지 않은 정점 사이는 무한대(INF)로 둔다. 자기 자신으로의 거리 D[i][i]는 0으로 초기화한다.
...
3중 반복문의 각 루프가 V번씩 실행되므로 총 V × V × V = V³번의 비교 및 갱신 연산이 수행된다. 각 연산은 덧셈, 비교, 최솟값 선택으로 이뤄지므로 상수 시간 O(1)에 처리된다. 공간 복잡도는 O(V²)이며, V × V 크기의 2차원 배열이 필요하다. 경로 복원을 위해 next 배열을 추가하더라도 전체 공간 복잡도는 여전히 O(V²)이다.
...
플로이드-워셜 알고리즘은 모든 정점 쌍 사이의 최단 경로를 한 번에 구해야 할 때 가장 직관적이고 강력한 선택지 중 하나다. O(V³)의 시간이 필요하지만, 구현이 단순하고 음수 가중치와 음수 사이클 처리까지 가능하다는 장점이 있다. 특히 정점 수가 수백 개 이하인 밀집 그래프에서는 실용성이 높으며, 추이적 폐쇄, 네트워크 지름 계산, 교통망 최적화, 게임 AI 같은 다양한 문제에 폭넓게 활용된다.
