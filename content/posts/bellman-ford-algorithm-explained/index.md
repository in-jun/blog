...
tags: ["알고리즘", "그래프", "최단경로"]
description: "음수 가중치 그래프에서 최단 경로를 찾는 벨만-포드 알고리즘을 설명한다."
draft: false
---
...
그래프에서 사이클을 포함하지 않는 단순 경로(Simple Path)는 최대 V-1개의 간선을 포함할 수 있다. 시작 정점에서 출발해 같은 정점을 두 번 이상 방문하지 않는 경로는 최대 V-1개의 간선만 거칠 수 있기 때문이다. 반대로 경로가 V개 이상의 간선을 포함하면 비둘기집 원리(Pigeonhole Principle)에 따라 반드시 같은 정점을 두 번 이상 방문하게 되고, 이는 곧 사이클의 존재를 뜻한다. 음수 사이클이 아니라면 그 사이클을 제거한 더 짧은 경로가 존재하므로, 그런 경로는 최단 경로가 될 수 없다.

따라서 첫 번째 반복에서는 시작 정점에서 최대 1개의 간선으로 도달할 수 있는 정점들의 최단 거리가 올바르게 계산된다. 두 번째 반복에서는 최대 2개의 간선으로 도달할 수 있는 정점들의 최단 거리가 계산된다. 같은 방식으로 i번째 반복이 끝나면 최대 i개의 간선을 사용하는 최단 경로가 확정된다. 이는 더 작은 하위 문제(더 적은 간선을 사용하는 최단 경로)의 해를 이용해 더 큰 문제(더 많은 간선을 사용하는 최단 경로)를 점진적으로 해결하는, 동적 프로그래밍의 전형적인 상향식(Bottom-up) 접근법이다.
...
벨만-포드 알고리즘은 V-1번의 반복 동안 모든 간선에 대한 완화를 반복적으로 수행하면서 최단 거리 값을 점진적으로 갱신한다. 다익스트라처럼 정점을 확정하는 개념이 없기 때문에, 음수 가중치로 인해 나중에 더 짧은 경로가 발견되더라도 다음 반복에서 그 값이 반영될 수 있다. 따라서 음수 사이클만 존재하지 않는다면 정확한 최단 거리를 찾을 수 있다.
...
벨만-포드 알고리즘에서는 V-1번의 반복이 끝나면 모든 최단 경로가 확정되어야 한다. 그런데 V번째 반복에서도 거리가 갱신되는 간선이 있다면, V개 이상의 간선을 사용하는 더 짧은 경로가 존재한다는 뜻이다. V개 이상의 간선을 사용하는 경로는 반드시 사이클을 포함한다. 그 상태에서도 거리가 계속 줄어든다면, 그 사이클은 음수 사이클이다. 따라서 알고리즘의 마지막에 모든 간선을 한 번 더 검사해 거리가 갱신되는지 확인하면, 음수 사이클의 존재 여부를 O(E) 시간에 판별할 수 있다.
...
SPFA는 벨만-포드 알고리즘의 최적화된 변형으로, 1994년 중국의 Duan Fanding이 제안했다. 이 알고리즘은 큐(Queue) 자료구조를 사용해 거리가 갱신된 정점만 선택적으로 처리한다. 모든 간선을 매 반복마다 검사하는 대신, 거리 값이 바뀐 정점의 인접 간선만 완화하므로 평균적으로는 O(E)에 가깝게 동작한다. 다만 최악의 경우 시간 복잡도는 여전히 O(VE)이다. 특히 특정 형태의 그래프(예: 격자 그래프)에서는 최악 사례가 발생할 수 있어, 최근에는 경쟁 프로그래밍에서 의도적으로 SPFA를 느리게 만드는 테스트 케이스가 포함되기도 한다.

## 결론

벨만-포드 알고리즘은 1950년대에 Richard Bellman과 Lester Ford Jr.가 독립적으로 발견한 알고리즘으로, 동적 프로그래밍의 원리를 그래프 최단 경로 문제에 적용한 대표적인 사례다. 시간 복잡도는 O(VE)로 다익스트라 알고리즘보다 일반적으로 느리지만, 음수 가중치를 가진 간선을 처리하고 그래프 내의 음수 사이클 존재 여부를 탐지할 수 있다는 뚜렷한 강점이 있다. 이런 특성 덕분에 RIP 라우팅 프로토콜, 금융 시장의 차익 거래 탐지, 최소 비용 흐름 문제, 제약 조건 차이 시스템처럼 특정 문제 영역에서 여전히 중요한 역할을 한다.
