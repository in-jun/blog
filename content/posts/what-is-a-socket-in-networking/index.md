...
소켓(Socket)은 네트워크 통신의 종단점(endpoint)을 추상화한 소프트웨어 인터페이스다. 1983년 UC Berkeley의 4.2BSD 유닉스 운영체제에서 처음 등장했으며, 오늘날까지 인터넷 통신의 근간을 이루는 핵심 기술로 자리 잡고 있다. 소켓은 IP 주소와 포트 번호의 조합으로 네트워크상의 고유한 통신 지점을 식별하고, 프로세스 간 데이터 교환을 위한 표준화된 API를 제공한다.
...
소켓은 크게 두 가지 역할로 나뉜다. 서버 소켓은 특정 IP 주소와 포트에서 클라이언트의 연결 요청을 수신 대기(listen)하고, 클라이언트 소켓은 서버 소켓에 연결(connect)을 시도해 통신 채널을 만든다. 연결이 수립되면 양쪽 모두 같은 방식으로 데이터를 주고받을 수 있다. 운영체제는 소켓을 파일 디스크립터로 관리하므로, 파일 입출력과 비슷한 방식으로 네트워크 통신을 처리할 수 있다.
...
> **TCP의 핵심 특성**
>
> TCP는 연결을 먼저 수립한 뒤 데이터를 주고받는 신뢰성 있는 전송 프로토콜이다. 3-way handshake로 연결을 맺고, 오류 감지와 재전송, 흐름 제어, 혼잡 제어를 통해 데이터가 순서대로 정확하게 도착하도록 보장한다.
...
> **UDP의 핵심 특성**
>
> UDP는 연결을 미리 수립하지 않고 데이터를 바로 보내는 전송 프로토콜이다. 전달 순서나 재전송을 보장하지 않지만 오버헤드가 작아 지연 시간이 짧고, 실시간성이 중요한 애플리케이션에 적합하다.
...
클라이언트와 서버는 같은 물리적 머신에서 실행될 수도 있고, 네트워크로 분리된 하드웨어에서 동작할 수도 있다. 또한 하나의 서버가 동시에 수천 개의 클라이언트 연결을 처리하는 일도 일반적이다. 서버는 보통 항상 실행된 상태로 클라이언트의 요청을 기다리고, 클라이언트는 필요할 때 서버에 연결해 서비스를 요청한 뒤 연결을 종료하는 패턴으로 동작한다.
...
WebSocket은 채팅 애플리케이션, 실시간 협업 도구(Google Docs, Figma), 주식 시세나 암호화폐 거래소처럼 서버가 클라이언트에 즉시 데이터를 보내야 하는 상황에서 널리 활용된다. 온라인 게임, 라이브 스포츠 스코어, IoT 대시보드 같은 실시간 서비스도 대표적인 예다. 기존의 롱 폴링(Long Polling)이나 Server-Sent Events(SSE)보다 지연 시간이 짧고, 양방향 통신을 지원한다.

## 마치며

소켓은 1983년 Berkeley Sockets로 시작해 40년 넘게 네트워크 프로그래밍의 표준 인터페이스로 자리 잡아 왔다. 오늘날에도 TCP와 UDP라는 두 전송 프로토콜 위에서 다양한 애플리케이션의 통신 요구를 충족시키고 있다. TCP는 신뢰성과 순서 보장이 필요한 곳에, UDP는 실시간성과 낮은 지연이 중요한 곳에 주로 사용되며, 이 선택이 애플리케이션의 성능과 특성을 크게 좌우한다. WebSocket은 이러한 저수준 소켓 개념을 웹 환경에 맞게 추상화해, 브라우저에서도 실시간 양방향 통신을 가능하게 만든 기술이다.
