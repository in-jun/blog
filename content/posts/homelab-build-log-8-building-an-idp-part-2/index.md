...
전통적인 CI/CD 파이프라인은 프로젝트마다 따로 손이 많이 갔지만, 내가 만들고 싶었던 내부 개발 플랫폼은 템플릿을 바탕으로 이런 반복 작업을 줄이는 데 초점을 맞췄다. 이 글에서 정리한 플랫폼은 대략 다음 흐름으로 동작한다:
...
6. **ArgoCD가 변경된 설정 파일을 감지해 새 이미지로 애플리케이션을 배포한다.**
...
CI 파이프라인이 프로젝트 설정 파일을 업데이트하면, 이후 배포는 ArgoCD가 변경 사항을 감지해 진행하는 구조였다.
...
실제로 새 프로젝트를 추가할 때는 아래와 같은 흐름으로 사용했다:

### 프로젝트 설정 파일 생성

`projects/myproject.yaml` 파일을 생성했다:
...
프로젝트 설정 파일을 커밋하고 푸시하면 ArgoCD가 관련 리소스를 자동으로 생성했다:
