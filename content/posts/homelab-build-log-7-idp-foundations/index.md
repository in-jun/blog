...
이번 글에서는 컨테이너 레지스트리, 이벤트 처리 시스템, 워크플로우 엔진을 각각 Harbor, Argo Events, Argo Workflows로 구성한다. 다음 글에서는 이 기반 위에 ArgoCD와 템플릿 구조를 더해 실제 IDP 형태로 정리한다.
...
Harbor도 이전 글들과 마찬가지로 GitOps 방식으로 배포했다. 먼저 Git 저장소에 Harbor용 디렉토리와 파일을 추가했다:
...
내 로컬 머신의 hosts 파일에는 아래 항목을 추가했다:

```
192.168.0.200 harbor.injunweb.com
```

웹 브라우저에서 `https://harbor.injunweb.com`에 접속해 `admin` 계정과 Vault에 저장된 비밀번호로 로그인했다. 로그인한 뒤 새 프로젝트를 만들고 Docker CLI에서 이미지 푸시도 테스트했다:
...
Argo Events도 같은 저장소에 추가했다. 먼저 디렉토리와 파일을 만들었다:
...
`values.yaml`은 기본 설정을 그대로 사용할 예정이라 비워 두었다:
...
Argo Workflows UI에 접근하기 위한 IngressRoute는 `templates/ingressroute.yaml`에 아래처럼 작성했다:
...
이번 글에서 구성한 세 가지 요소(Harbor, Argo Events, Argo Workflows)만으로도 IDP의 기반은 갖춰졌지만, 개발자가 바로 활용할 수 있는 형태로 만들려면 다음 작업이 더 필요했다:
