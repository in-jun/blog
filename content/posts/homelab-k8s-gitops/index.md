...
이 명령은 Helm 공식 설치 스크립트를 내려받아 실행하는 방식이다. 설치가 끝난 뒤에는 아래 명령으로 버전이 정상인지 확인했다:
...
이 명령은 시크릿에서 base64로 인코딩된 비밀번호를 꺼내 디코딩해 보여준다. 나는 이 값을 바로 기록해두고, 로그인 후에는 별도 비밀번호로 바꿨다.
...
ArgoCD를 제대로 쓰려면 저장소 구조부터 정리할 필요가 있었다. 내 구성에서는 역할을 나누기 위해 저장소를 두 개로 나눴다:
...
루트 Application은 `app-of-apps.yaml`로 아래처럼 저장했다:
...
구성이 끝난 뒤 내 GitOps 워크플로우는 아래 순서로 굴러갔다:
