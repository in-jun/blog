...
전통적인 인프라 관리 방식에서는 관리자가 서버에 직접 접속해 명령어를 실행하거나 콘솔에서 설정을 변경하곤 했다. 이 방식은 변경 이력을 추적하기 어렵고, 실수로 장애가 발생했을 때 원인 파악과 복구가 복잡하며, 여러 환경 간의 일관성을 유지하기 어렵다는 문제가 있었다. GitOps는 이러한 문제를 해결하기 위해 모든 인프라 구성을 코드로 정의하고 Git 저장소에서 버전 관리한다. 자동화된 도구가 Git 저장소의 상태와 실제 클러스터의 상태를 지속적으로 비교해 차이가 발생하면 자동으로 동기화하는 방식으로 운영한다.
...
> **ArgoCD란?**
>
> ArgoCD는 쿠버네티스를 위한 선언적 GitOps 배포 도구로, 2018년 Intuit에서 개발하여 오픈소스로 공개되었으며 현재 CNCF(Cloud Native Computing Foundation)의 졸업 프로젝트로 널리 사용되고 있다. Git 저장소에 정의된 쿠버네티스 매니페스트를 자동으로 클러스터에 동기화하고, 웹 UI와 CLI를 통해 애플리케이션 상태를 시각적으로 모니터링할 수 있는 기능을 제공한다.
...
ArgoCD를 설치하는 방법은 몇 가지가 있지만, 내 홈랩에서는 쿠버네티스 패키지 관리자인 Helm을 사용하는 쪽이 가장 단순했다.
...
이 명령은 Helm 공식 설치 스크립트를 내려받아 실행하는 방식이다. 설치가 끝난 뒤에는 아래 명령으로 버전이 정상적으로 출력되는지 확인했다:
...
ArgoCD는 전용 네임스페이스를 따로 만들어 분리했다:
...
`upgrade --install` 옵션은 ArgoCD가 이미 설치되어 있으면 업그레이드하고, 설치되어 있지 않으면 새로 설치하는 명령어로 동일한 명령어를 반복 실행해도 같은 결과를 보장한다. 설치가 완료되면 다음과 같은 메시지가 출력된다:
...
초기 관리자 비밀번호는 쿠버네티스 시크릿에 들어 있어서 아래 명령으로 꺼냈다:

```bash
kubectl -n argocd get secret argocd-initial-admin-secret -o jsonpath="{.data.password}" | base64 -d; echo
```

이 명령은 시크릿에서 base64로 인코딩된 비밀번호를 꺼내 디코딩해 보여준다. 이 값은 바로 기록해두고, 로그인 후에는 별도 비밀번호로 바꿨다.

### 웹 UI 접속

처음 접속은 간단하게 포트 포워딩으로 열었다:
...
ArgoCD를 제대로 쓰려면 저장소 구조부터 정리할 필요가 있었다. 내 구성에서는 부트스트랩 로직과 실제 리소스 정의를 분리하기 위해 저장소를 두 개로 나눴다:
...
이렇게 나누면 관리 복잡도를 줄일 수 있고, 저장소별로 접근 권한을 다르게 설정하기도 수월하다.
...
내가 사용한 App of Apps 패턴은 대략 다음 흐름으로 움직인다:
...
루트 Application은 `app-of-apps.yaml`이라는 이름으로 아래처럼 저장했다:
...
`app-of-apps` 저장소의 `templates/infra-apps-root.yaml`은 아래처럼 두었다:
...
이 Application은 `k8s-resource` 저장소의 루트 디렉토리에서 `applicationset.yaml` 파일만 가져와 적용한다. `directory.include` 설정으로 특정 파일만 포함할 수 있다.
...
`k8s-resource` 저장소의 `applicationset.yaml`은 아래와 같다:
...
구성이 끝난 뒤 내 GitOps 워크플로우는 아래 순서로 진행됐다:
...
이 구조를 만든 뒤부터는 `k8s-resource` 저장소의 `apps/` 아래에 새 폴더를 추가하고 커밋하면 ArgoCD가 자동으로 감지해 배포했다. 기존 애플리케이션도 같은 방식으로 파일만 수정해 커밋하면 변경이 반영됐다.
