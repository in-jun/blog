...
이 매니페스트는 ArgoCD가 첫 번째 Git 저장소(app-of-apps)를 모니터링하여 변경 사항이 있을 때마다 자동으로 동기화하도록 설정한다. `syncPolicy.automated` 섹션의 설정으로 인해 삭제된 리소스도 자동으로 제거된다.

또한 수동으로 변경된 리소스도 원래 상태로 복구된다.
