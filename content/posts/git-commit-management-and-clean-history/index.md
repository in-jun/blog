...
Git의 커밋 관리 기능은 2005년 Linus Torvalds가 Git을 개발할 때부터 핵심 설계 원칙 가운데 하나였다. 특히 rebase는 초기 버전부터 제공됐고, 2007년 Git 1.5에서 interactive rebase가 도입되면서 커밋 히스토리를 세밀하게 편집할 수 있는 강력한 도구가 되었다.

커밋 히스토리 관리가 중요한 이유는 Git 로그가 프로젝트의 변경 이력을 담은 문서 역할을 하기 때문이다. 잘 정리된 히스토리는 `git log`만으로도 프로젝트의 발전 과정을 파악하게 해주고, `git bisect`로 버그를 추적할 때 각 커밋의 의도를 더 분명하게 이해하게 해준다. 새로운 팀원이 합류했을 때도 코드베이스의 변화를 빠르게 따라갈 수 있다.
...
단일 책임 커밋이 중요한 이유는 분명하다. 코드 리뷰에서는 리뷰어가 한 번에 하나의 변경에만 집중할 수 있어 품질이 높아진다. 문제가 발생했을 때는 `git revert`로 특정 변경만 되돌릴 수 있어 롤백이 쉬워진다. `git cherry-pick`으로 특정 기능만 다른 브랜치로 가져가기에도 유리하고, `git bisect`로 버그 원인을 찾을 때도 범위를 좁히기 쉽다.
...
작업 중인 상태를 커밋할 때는 나중에 정리할 것을 전제로 `WIP:` 접두사를 사용한다. 이런 커밋은 PR 전에 squash하거나 rebase로 정리한다.
...
`git rebase`는 Git 초기 버전부터 존재했다. interactive rebase(`git rebase -i`)는 2007년 Git 1.5.4에서 도입됐으며, Johannes Schindelin이 개발했다. 이 기능은 커밋 히스토리를 세밀하게 편집할 수 있어 지금도 Git 워크플로우의 핵심 도구로 쓰인다.
...
Git 1.7.4에서 도입된 `--autosquash` 옵션은 커밋 메시지에 `fixup!`이나 `squash!` 접두사가 있으면 해당 커밋을 자동으로 재배치한다. 이를 활용하면 나중에 정리할 커밋을 미리 표시해둘 수 있다.
...
`--amend`와 `rebase`는 커밋의 SHA-1 해시를 변경한다. 이미 원격에 push한 커밋에 사용하면 히스토리가 갈라져 `--force` push가 필요해질 수 있다. 이런 상황은 협업 중인 다른 팀원의 로컬 히스토리와 충돌을 일으킬 수 있으므로, 공유된 브랜치에서는 사용을 피해야 한다.
...
Git 커밋 관리 기법은 2005년 Git 탄생 이후 interactive rebase(2007), autosquash(2010) 같은 기능이 더해지며 계속 발전해왔다. 핵심 원칙은 단일 책임 원칙을 커밋에도 적용해 하나의 논리적 변경만 담고, 로컬에서는 자주 커밋하되 push 전에 정리하며, interactive rebase와 fixup/squash로 히스토리를 깔끔하게 유지하는 것이다.

클린 코드만큼이나 클린 히스토리도 중요하다. 잘 정리된 커밋 히스토리는 코드 리뷰, 버그 추적, 새 팀원 온보딩 같은 작업 전반에서 프로젝트의 품질을 높인다.
