...
tags: ["JPA", "ORM", "성능"]
description: "JPA의 지연 로딩과 즉시 로딩의 차이와 성능에 미치는 영향을 설명한다."
draft: false
---

## 로딩 전략의 역사와 필요성

ORM(Object-Relational Mapping) 프레임워크는 객체지향 프로그래밍과 관계형 데이터베이스 사이의 불일치를 해결하기 위해 등장했다. 이 과정에서 연관관계를 가진 엔티티를 어떻게 효율적으로 로딩할 것인가는 중요한 과제가 되었다. 초기 ORM 구현체들은 모든 연관 엔티티를 즉시 로딩하는 방식을 사용했지만, 이는 불필요한 데이터까지 메모리에 적재해 성능 저하를 일으켰다. Hibernate는 이를 해결하기 위해 프록시 기반의 지연 로딩 메커니즘을 도입했다.

Hibernate는 2001년 처음 공개된 이후 로딩 전략을 지속적으로 개선해왔다. 초기 버전에서는 단순한 지연 로딩만 지원했지만, 이후 fetch join, batch fetching, subselect fetching 등 다양한 최적화 기법이 추가되었다. 이러한 기능들은 2006년 공개된 JPA(Java Persistence API) 1.0 표준에 반영되었고, JPA 2.0(2009년)과 JPA 2.1(2013년)을 거치며 @EntityGraph 같은 선언적 페치 전략도 도입되었다. 현재는 JPA 표준과 Hibernate 구현체가 함께 발전하면서 개발자에게 다양한 로딩 전략을 제공하고 있다.
...
실제 엔티티와 프록시 객체는 겉보기에는 동일해 보이지만, instanceof 연산자나 getClass() 메서드를 사용할 때 차이가 드러난다. 따라서 엔티티를 비교할 때는 반드시 equals() 메서드를 사용해야 하며, 프록시 여부를 확인하려면 Hibernate.isInitialized() 메서드나 PersistenceUnitUtil.isLoaded() 메서드를 활용할 수 있다.
...
fetch join은 SQL의 INNER JOIN이나 LEFT OUTER JOIN을 사용해 연관 엔티티를 함께 조회하므로 쿼리가 1번만 실행된다. 다만 컬렉션 페치 조인에서는 중복 결과가 발생할 수 있으므로 distinct 키워드를 함께 사용하는 것이 좋다. 또한 페이징 API(setFirstResult, setMaxResults)와 함께 사용하면 경고 로그가 출력되고 메모리에서 페이징이 처리되므로 주의해야 한다.
...
@EntityGraph는 LEFT OUTER JOIN을 사용해 연관 엔티티를 로딩한다. 또한 attributePaths에 여러 연관 엔티티를 지정할 수 있고, @NamedEntityGraph와 조합해 재사용 가능한 그래프를 정의할 수도 있다. 다만 복잡한 조건이나 동적 쿼리에는 fetch join이 더 적합하다.
...
@BatchSize를 사용하면 프록시 초기화 시 지정된 크기만큼의 엔티티를 WHERE IN 절로 한 번에 조회하므로 N+1 문제를 크게 완화할 수 있다. 예를 들어 1000개의 회원이 있고 배치 크기가 100이면, 회원 조회 1번과 주문 조회 10번으로 총 11번의 쿼리가 실행된다. 글로벌 설정으로 hibernate.default_batch_fetch_size를 사용하면 모든 엔티티에 일괄 적용할 수도 있다.
...
이러한 기본값 설계는 단일 엔티티(ToOne)는 조인 비용이 적고 함께 사용될 가능성이 높아 즉시 로딩하고, 컬렉션(ToMany)은 데이터 양이 많고 사용되지 않을 수 있어 지연 로딩하는 것이 합리적이라는 판단에 기반한다. 다만 실무에서는 모든 연관관계를 LAZY로 설정하고, 필요한 경우에만 fetch join을 사용하는 방식이 권장된다.

## 실전 권장사항

실무에서는 모든 연관관계에 FetchType.LAZY를 기본으로 사용하는 경우가 많다. 특정 화면이나 API에서 연관 엔티티가 필요할 때만 fetch join이나 @EntityGraph를 사용해 선택적으로 즉시 로딩하는 전략이 가장 효과적이다. 반면 EAGER는 예측하기 어려운 쿼리를 발생시키고, JPQL에서는 N+1 문제를 일으킬 수 있으므로 가급적 사용하지 않는 것이 좋다.

Open Session In View(OSIV) 패턴은 영속성 컨텍스트를 뷰 레이어까지 열어두어 지연 로딩을 편리하게 사용할 수 있게 한다. 하지만 데이터베이스 커넥션을 오랫동안 점유하므로, 실시간 트래픽이 많은 애플리케이션에서는 커넥션 부족 문제를 일으킬 수 있다. 따라서 Spring Boot의 spring.jpa.open-in-view 옵션을 false로 설정하고, 트랜잭션 내에서 필요한 데이터를 모두 로딩하는 방식을 권장한다.

트랜잭션 범위 밖에서 지연 로딩을 시도하면 LazyInitializationException이 발생한다. 이를 방지하려면 서비스 계층의 트랜잭션 안에서 fetch join이나 프록시 초기화를 통해 필요한 데이터를 미리 로딩해야 한다. 또는 DTO로 변환해 뷰에 전달하는 방식도 사용할 수 있으며, 이 방법은 영속성 컨텍스트와 프레젠테이션 계층을 분리한다는 관점에서도 바람직하다.
