...
i3 윈도우 매니저(i3wm)는 효율성과 생산성에 초점을 맞춘 타일링 윈도우 매니저다. 2009년 독일의 개발자 Michael Stapelberg가 기존 wmii 윈도우 매니저의 한계를 보완하기 위해 개발했으며, 이름은 "improved tiling wm"에서 따왔다. C 언어로 작성된 i3는 약 10,000줄 규모의 비교적 작은 코드베이스로 이루어져 있으며, 마우스보다 키보드 중심의 워크플로우를 선호하는 개발자, 시스템 관리자, 파워 유저 사이에서 널리 쓰인다. Arch Linux, Gentoo, NixOS 사용자층에서도 자주 언급되는 편이다.

i3는 전통적인 X Window System(X11) 환경에서 동작하며 디스플레이 매니저와 독립적으로 실행되기 때문에 LightDM, GDM, SDDM 같은 로그인 매니저와 함께 사용할 수 있다. 최근에는 Wayland를 지원하는 호환 구현인 Sway(i3-compatible Wayland compositor)도 널리 쓰이며, i3의 설정과 키 바인딩을 상당 부분 그대로 활용할 수 있다. i3는 "do one thing and do it well"이라는 Unix 철학에 가깝게 불필요한 시각 효과를 줄이고 화면 공간을 작업 콘텐츠에 집중하도록 설계되었다. 여기에 다중 모니터 지원, 효율적인 창 관리, 텍스트 기반 설정 파일을 통한 사용자 정의 기능을 제공한다.

i3wm은 공식 문서가 잘 정리되어 있어 타일링 윈도우 매니저를 처음 접하는 사용자도 단계적으로 기능을 익히고 환경을 구성하기 좋다. 공식 문서는 [i3wm.org/docs](https://i3wm.org/docs/)에서 확인할 수 있으며, User's Guide, Configuration Reference, IPC Protocol 같은 주제를 다룬다. 커뮤니티 포럼과 Reddit의 r/i3wm 서브레딧에서도 관련 정보를 찾아볼 수 있다.
...
i3의 타일링 알고리즘은 이진 트리(Binary Tree) 구조를 기반으로 한다. 사용자가 새 창을 열 때마다 현재 포커스된 창이 차지하던 공간이 트리 노드로서 둘로 분할(Split)되고, 하나는 기존 창이, 다른 하나는 새 창이 차지하게 된다. 분할 방향(수평 또는 수직)은 사용자가 명시적으로 지정하거나 i3의 기본 동작 모드에 따라 결정된다. 이런 트리 구조 덕분에 i3는 복잡한 레이아웃도 일관되게 관리할 수 있으며, 창의 부모-자식 관계가 분명해 창을 이동하거나 제거할 때도 예측 가능한 동작을 보인다.
...
설치가 끝나면 로그아웃한 뒤 로그인 화면에서 i3 세션을 선택해 시작하면 된다. 첫 실행 시에는 설정 마법사가 나타나 설정 파일(`~/.config/i3/config`)을 만들지와 mod 키를 무엇으로 할지 묻는다. mod 키는 i3의 모든 단축키에서 사용하는 핵심 수정자 키로, 보통 Alt(Mod1) 또는 윈도우 키/Super 키(Mod4) 중 하나를 선택한다. Alt 키는 다른 애플리케이션 단축키와 겹칠 수 있어 나는 윈도우 키(Mod4)를 더 자주 사용한다.
...
i3wm은 vim 편집기와 달리 기본적으로 `jkl;`를 방향키로 사용한다. 이는 키보드 홈 로우(Home Row)에서 오른손의 자연스러운 위치를 고려한 설계지만, vim에 익숙한 사용자에게는 다소 낯설 수 있다. 필요하면 설정 파일(`~/.config/i3/config`)에서 `hjkl` 스타일로 바꿀 수 있다. 나 역시 vim과 tmux, 여러 CLI 도구에서 `hjkl`에 익숙해져 있어 i3에서도 같은 배열을 사용하고 있다.
...
나의 설정에서는 `"1"`, `"2"`, `"3"`처럼 숫자만 있는 간결한 워크스페이스 이름을 사용한다. 빠르게 전환할 수 있고 인지적 부담이 적다고 느끼기 때문이다. 일부 사용자는 `"1:web"`, `"2:code"`, `"3:term"`처럼 설명을 붙여 각 워크스페이스의 용도를 드러내기도 한다. 하지만 나는 워크스페이스를 고정된 용도보다 유동적으로 쓰는 편이라 숫자만 두는 방식이 더 잘 맞았다. 필요하면 설정 파일에서 언제든 이름을 바꿀 수 있다는 점도 장점이다.

## i3bar와 i3status 커스터마이징

i3wm은 화면 하단이나 상단에 상태 표시줄을 둘 수 있다. 이 표시줄은 i3bar와 i3status로 구성되며, i3bar는 표시를 담당하고 i3status는 시스템 정보를 수집해 형식에 맞춰 출력한다. i3status는 CPU 사용률, 메모리 사용량, 디스크 공간, 네트워크 상태, 배터리 잔량, 시간 등을 모니터링하며, 설정 파일(`~/.config/i3status/config`)에서 표시 항목과 형식을 조정할 수 있다. 더 많은 기능이 필요하다면 i3status 대신 i3blocks, polybar, bumblebee-status 같은 대체 상태 표시줄 프로그램을 사용할 수도 있다.
...
이 설정에서는 바를 평소에 숨겨 두었다가 `$mod` 키를 누를 때만 표시하고, 색상도 다크 테마에 맞춰 구성했다.
...
i3wm은 GNOME, KDE Plasma, Xfce 같은 전통적인 데스크톱 환경과는 다른 방식으로 동작하는 미니멀한 윈도우 매니저다. 초반에는 학습 곡선이 있지만, 익숙해지면 마우스 사용을 줄이고 키보드만으로 빠르게 작업할 수 있다. 키보드 중심 인터페이스, 자동 타일링을 통한 창 관리, 텍스트 기반 설정 파일이 주는 사용자화 가능성, 그리고 경량성은 개발자와 시스템 관리자, 파워 유저에게 특히 잘 맞는다. 멀티태스킹이 많은 개발 워크플로우에서도 장점이 분명하다.

학습 곡선이 다소 가파르게 느껴질 수 있지만 참고할 자료는 충분하다. 공식 문서([i3wm.org/docs](https://i3wm.org/docs/))는 단계적으로 학습하기 좋고, Reddit의 r/i3wm 커뮤니티와 Arch Wiki의 i3 페이지에서도 다양한 예제와 팁을 얻을 수 있다. 무엇보다 직접 사용하면서 설정을 조금씩 개선해나가는 과정 자체가 재미있고 보람 있는 경험이 된다.
