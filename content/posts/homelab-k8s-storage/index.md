...
Longhorn을 올리기 전에 나는 모든 노드에서 아래 요구사항을 먼저 확인했다:
...
패키지 설치 뒤에는 각 노드에서 iSCSI 서비스가 정상인지 확인했다:
...
Longhorn 설정은 GitOps 저장소에 추가했고, 로컬에서는 먼저 저장소를 클론한 뒤 디렉토리를 만들었다:
...
`values.yaml`에는 Longhorn에서 실제로 쓸 값을 아래처럼 정리했다:
...
푸시 후에는 ApplicationSet이 애플리케이션을 제대로 만들었는지 확인했다:
...
검증은 아래 매니페스트를 적용한 뒤 상태를 보는 방식으로 진행했다:
...
이번 글에서는 내 홈랩 쿠버네티스 클러스터에 Longhorn 분산 스토리지를 붙인 과정을 정리했다. Longhorn은 로컬 스토리지나 NFS의 한계를 피하면서도 Rook-Ceph보다 가볍게 가져갈 수 있어서, 당시 내 환경에는 가장 현실적인 선택이었다.
