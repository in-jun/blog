...
ERD(Entity Relationship Diagram)는 데이터베이스 구조를 시각적으로 표현한 다이어그램으로, 테이블 간 관계를 한눈에 보여 주어 설계와 구조를 이해하기 쉽게 돕는다.

![ERD 기본 개념도](image.png)

Entity-Relationship 모델링은 Peter Chen이 데이터베이스 설계를 위해 개발했으며, 1976년 논문으로 발표되었다. 그는 MIT Sloan School of Management에서 조교수로 재직하던 시기에 "The Entity-Relationship Model: Toward a Unified View of Data"라는 논문을 발표했고, 이 논문은 ACM Transactions on Database Systems에 게재되어 오늘날까지도 자주 인용된다.

현재 미국 피츠버그의 카네기멜런대학교 교수인 Peter Chen(Peter Pin-Shan Chen)은 1970년대에 데이터베이스 설계를 위한 ER 모델링을 정립한 인물로 평가받는다. Chen의 작업 이전에도 Charles Bachman과 A.P.G. Brown이 유사한 연구를 진행했으며, Bachman은 Data Structure Diagram의 한 유형인 Bachman Diagram을 개발했다.
...
ERD는 엔티티(Entity), 속성(Attribute), 관계(Relationship)의 세 가지 핵심 구성 요소로 이루어져 있으며, 이들은 데이터베이스 설계의 기본 요소를 이룬다.
...
Crow's Foot(Information Engineering/IE) 표기법에서는 엔티티를 속성이 포함된 상자로 표현하고, 관계는 카디널리티(최대)와 옵셔널리티(최소)를 나타내는 기호로 표시한다. Crow's Foot 표기법은 간결하고 읽기 쉬우며 관계형 데이터베이스 모델에 직접 매핑될 수 있다.

![Crow's Foot 표기법 예시](image-2.png)

장점으로는 간결하고 읽기 쉬우며 CASE 도구에서 폭넓게 지원되고 관계형 스키마에 쉽게 매핑할 수 있다는 점이 있다. 실무에서는 IE와 Barker 같은 간결한 표기법이 널리 사용된다.
...
모델이 커질수록 의사소통에는 간결한 표기법(Crow's Foot)을 우선 사용하고, 교육이나 표준 문서처럼 세부 표현이 필요한 경우에만 상세한 표기법(Chen/IDEF1X)을 쓰는 것이 좋다. 실무에서는 IE(Crow's Foot) 표기법이 가장 널리 사용된다.
...
정규화는 ERD 설계 과정에서 데이터 중복을 최소화하고 데이터 무결성을 보장하기 위한 핵심 원칙이다. ERD를 설계하는 단계부터 정규화를 함께 고려하면 요구사항을 더 명확하게 분석할 수 있다.

### 정규화와 ERD의 동시 적용

정규화와 ER 모델링은 서로 밀접하게 연결되어 있어 완전히 분리해 진행하기 어렵다. 최근의 실무에서는 ERD를 설계하는 과정에서 정규화 원칙을 함께 적용하며, 이를 별도의 순차 단계로 떼어 두지 않는다.

### 제3정규형 (3NF) 원칙

각 엔티티 타입에서는 엔티티 식별자가 모든 비식별자 속성을 결정해야 하며, 비식별자 속성들 사이에는 추가 종속성이 없어야 한다. 이는 제3정규형(3NF)의 핵심 원칙이다.
...
개발자, DBA, 데이터 분석가를 위해 설계된 Domain-specific language(DSL) 기반의 빠르고 간단한 ER 다이어그램 도구이다. 사용자는 키보드만으로 ERD를 만들 수 있고 인터페이스도 단순하고 직관적이다.

요청하면 데이터베이스 객체 생성용 SQL 스크립트도 만들 수 있으며, 키보드 기반 워크플로를 선호하는 개발자에게 특히 잘 맞는다. 가격은 최대 10개의 다이어그램까지 무료이고 그 이후에는 월 $9이다.

### Draw.io (Diagrams.net)

회원가입 없이도 Entity Relationship Diagram을 만들 수 있는 가장 인기 있는 ER 모델 도구 중 하나이다. 오픈 소스이며 브라우저 기반이고 오프라인에서도 작동한다.
...
무료이며 가입이 필요 없고 SQL과 PNG를 내보낼 수 있다. 다만 시각적 표현이 매우 기본적이고 실시간 협업 기능도 없어 대규모 스키마 작업에는 적합하지 않다.
