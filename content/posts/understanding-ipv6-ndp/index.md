...
NDP(Neighbor Discovery Protocol)는 2007년 IETF RFC 4861로 표준화된 IPv6 네트워크의 핵심 프로토콜이다. IPv4에서 각각 ARP(Address Resolution Protocol), ICMP Router Discovery, ICMP Redirect 등이 맡던 기능을 ICMPv6(Internet Control Message Protocol version 6) 위에서 통합해, 이웃 노드 탐색, 라우터 발견, 주소 자동 구성, 주소 중복 검사, 경로 최적화 같은 작업을 처리한다.

IPv4 환경에서는 ARP, DHCP, ICMP 등 여러 프로토콜이 독립적으로 동작해 네트워크 관리가 복잡했다. 반면 IPv6에서는 NDP가 이 기능들을 통합해 프로토콜 스택을 단순화하고 보안 기능을 확장했다. 특히 멀티캐스트(multicast) 기반 통신을 사용해 IPv4의 브로드캐스트(broadcast) 방식보다 불필요한 트래픽을 줄일 수 있으며, 이는 대규모 네트워크 환경에서 효율과 전력 사용 측면에서도 이점이 있다.
...
- 라우터는 Router Advertisement (RA) 메시지로 응답하거나 주기적으로(기본 200초마다) 멀티캐스트
- RA 메시지에는 네트워크 프리픽스, 기본 게이트웨이 정보, MTU, 주소 구성 플래그 등 포함

### 2. 주소 자동 구성 (Stateless Address Autoconfiguration, SLAAC)

호스트는 DHCP 서버 없이도 라우터로부터 받은 프리픽스와 자신의 인터페이스 ID를 결합해 글로벌 유니캐스트 주소를 자동으로 생성할 수 있다. 이 방식은 대규모 네트워크에서 주소 관리 부담을 줄이고 플러그 앤 플레이(Plug and Play) 방식의 네트워크 연결을 가능하게 한다.
...
SLAAC는 DHCP 서버 없이도 호스트가 자동으로 IPv6 주소를 구성할 수 있게 하는 기능이다. 대규모 네트워크나 IoT 환경에서 관리 부담을 줄이고 플러그 앤 플레이 방식의 네트워크 연결을 가능하게 한다.
...
공격자가 위조된 RA 메시지를 전송해 호스트의 네트워크 구성을 조작하는 공격으로, 기본 게이트웨이와 프리픽스 정보에 직접 영향을 주기 때문에 대표적인 NDP 위협으로 꼽힌다.
...
SEND는 NDP 메시지에 암호화 서명을 추가해 인증과 무결성을 보장하는 프로토콜이다. 공개 키 암호화(RSA) 기반 인증을 사용하며, RA 스푸핑과 NS/NA 스푸핑을 줄이는 데 도움이 된다.
...
NDP(Neighbor Discovery Protocol)는 IPv6 네트워크에서 주소 구성, 이웃 탐색, 라우터 발견, 경로 최적화를 담당하는 핵심 프로토콜이다. 멀티캐스트 기반 통신과 SLAAC를 통해 IPv4보다 효율적이고 자동화된 네트워크 운영을 가능하게 하지만, 기본적으로는 인증 메커니즘이 부족해 RA 스푸핑, NS/NA 스푸핑, DAD DoS, Redirect 공격 같은 위협에도 노출될 수 있다. 따라서 실제 운영 환경에서는 SEND, RA Guard, DHCPv6 Guard, IPv6 Source Guard, ACL, VLAN 분할 같은 보호 기법을 함께 적용해 NDP의 장점을 안전하게 활용해야 한다.
