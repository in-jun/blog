...
JPA(Java Persistence API)에서 엔티티의 생명주기는 엔티티 객체가 생성되고 소멸할 때까지 거치는 상태 변화를 뜻한다. 이 개념은 2006년 Java EE 5와 함께 발표된 JPA 1.0 명세에서 처음 정의되었으며, Hibernate의 Session 개념을 표준화한 영속성 컨텍스트(Persistence Context)를 중심으로 설계되었다. 엔티티는 영속성 컨텍스트의 관리 여부와 데이터베이스 동기화 상태에 따라 비영속(Transient), 영속(Managed), 준영속(Detached), 삭제(Removed)의 4가지 상태로 나뉜다. 각 상태는 EntityManager의 persist(), merge(), detach(), remove() 메서드를 통해 전이된다.

엔티티의 상태를 올바르게 이해하는 것은 JPA를 효과적으로 사용하고 데이터 일관성을 유지하며 성능을 최적화하는 데 필수적이다. 영속성 컨텍스트가 제공하는 1차 캐시, 변경 감지(Dirty Checking), 쓰기 지연(Write-Behind) 같은 기능은 모두 엔티티가 영속 상태일 때만 제대로 작동하기 때문이다. 상태를 잘못 이해하면 LazyInitializationException이나 EntityNotFoundException 같은 예외가 발생하거나, 의도치 않은 데이터 손실로 이어질 수 있다.

## 엔티티 생명주기의 역사적 배경

JPA의 엔티티 생명주기 개념은 2001년 Gavin King이 개발한 Hibernate의 Session과 영속 객체(Persistent Object) 개념에서 유래했다. 당시 Hibernate는 EJB 2.x의 Entity Bean이 가진 복잡성과 성능 문제를 해결하기 위해 POJO(Plain Old Java Object) 기반의 단순한 영속화 모델을 제시했다. 또한 객체의 상태를 추적하고 변경 사항을 자동으로 데이터베이스에 반영하는 투명한 영속화(Transparent Persistence) 개념을 도입했다. 이 아이디어는 2006년 JPA 1.0 표준으로 채택되어 EntityManager와 영속성 컨텍스트라는 이름으로 명세화되었고, 이후 JPA 2.0(2009년), JPA 2.1(2013년), JPA 2.2(2017년)를 거치며 발전해왔다.
...
영속 상태의 엔티티는 1차 캐시에 저장된다. 따라서 같은 트랜잭션 내에서 동일한 식별자로 다시 조회하면 데이터베이스에 접근하지 않고 캐시에서 반환된다. 변경 감지 기능도 활성화되어 엔티티의 필드를 수정하기만 해도 트랜잭션 커밋 시 자동으로 UPDATE 쿼리가 실행된다. 또한 지연 로딩(Lazy Loading)을 통해 연관 엔티티를 필요한 시점에 로딩할 수 있고, 쓰기 지연 저장소에 SQL을 모아두었다가 한 번에 실행해 데이터베이스 왕복 횟수를 줄일 수 있다.
...
준영속 상태는 이전에 영속 상태였던 엔티티가 영속성 컨텍스트에서 분리되어 더 이상 EntityManager의 관리를 받지 않는 상태다. 이 상태의 엔티티는 데이터베이스 식별자(ID)는 가지고 있지만 영속성 컨텍스트의 관리 대상이 아니므로 변경 감지, 지연 로딩, 1차 캐시 같은 기능이 동작하지 않는다. detach() 메서드로 특정 엔티티만 분리하거나, clear()로 영속성 컨텍스트 전체를 초기화하거나, close()로 영속성 컨텍스트를 종료하면 엔티티는 준영속 상태로 전환된다. 트랜잭션이 종료되어 영속성 컨텍스트가 닫히는 경우에도 관리되던 엔티티들은 모두 준영속 상태가 된다.
...
변경 감지는 영속 상태의 엔티티가 수정되었을 때 별도의 update() 메서드 호출 없이 자동으로 변경 사항을 감지해 UPDATE 쿼리를 생성하는 기능이다. 영속성 컨텍스트는 엔티티를 처음 영속 상태로 만들 때 스냅샷을 저장해두고, flush 시점에 현재 엔티티와 스냅샷을 비교해 변경된 필드를 찾는다. 기본적으로는 변경된 필드만이 아니라 모든 필드를 UPDATE하는 쿼리가 생성된다. 이는 쿼리를 미리 준비하고 재사용할 수 있어 애플리케이션 로딩 시점에 쿼리를 파싱하고 캐싱하기 쉽기 때문이다. 필요하다면 @DynamicUpdate 어노테이션을 사용해 변경된 필드만 UPDATE하도록 할 수 있다.

### Flush와 Clear

flush는 영속성 컨텍스트의 변경 내용을 데이터베이스에 반영하는 작업이다. em.flush()를 직접 호출할 수도 있고, 트랜잭션 커밋 시 자동으로 호출되기도 한다. JPQL 쿼리 실행 전에도 자동으로 호출되어 쿼리 결과의 일관성을 보장한다. flush가 실행되면 변경 감지가 작동해 수정된 엔티티를 찾고, 수정 쿼리를 쓰기 지연 SQL 저장소에 등록한 뒤 데이터베이스에 전송한다. 중요한 점은 flush가 영속성 컨텍스트를 비우는 작업이 아니라 변경 내용을 동기화하는 작업이라는 것이다. 따라서 flush 후에도 엔티티는 여전히 영속 상태를 유지한다.

반면 clear는 영속성 컨텍스트를 완전히 초기화해 관리 중인 모든 엔티티를 준영속 상태로 전환하는 작업이다. em.clear()를 호출하면 1차 캐시를 포함한 영속성 컨텍스트의 모든 내용이 삭제된다. 그래서 배치 처리 시 메모리 사용량을 관리하거나 테스트 코드에서 격리를 보장할 때 자주 사용한다. clear 후에는 이전에 조회했던 엔티티를 다시 사용하려면 재조회해야 하며, 기존 엔티티 객체에 대한 변경 사항은 데이터베이스에 반영되지 않는다.
...
merge() 메서드는 준영속 상태의 엔티티를 다시 영속성 컨텍스트와 연결할 때 사용하는 메서드다. 먼저 준영속 엔티티의 식별자로 영속성 컨텍스트에서 엔티티를 조회하고, 없으면 데이터베이스에서 조회한다. 그런 다음 조회된 영속 엔티티에 준영속 엔티티의 값을 복사하고 병합된 영속 엔티티를 반환한다. 중요한 점은 merge()가 준영속 엔티티 자체를 영속 상태로 바꾸는 것이 아니라, 그 값을 반영한 새로운 영속 엔티티를 반환한다는 점이다. 따라서 merge() 호출 후에는 반환된 엔티티를 사용해야 하며, 원본 준영속 엔티티는 여전히 준영속 상태로 남아있다.
...
배치 처리 시에는 일정 개수의 엔티티를 처리한 후 flush()와 clear()를 호출해 메모리 사용량을 관리해야 한다. 예를 들어 10,000건의 데이터를 삽입할 때 100건마다 flush()와 clear()를 호출하면, 영속성 컨텍스트에 최대 100개의 엔티티만 유지되어 OutOfMemoryError를 방지할 수 있다.

## 결론

JPA 엔티티의 생명주기는 비영속, 영속, 준영속, 삭제의 4가지 상태로 구분된다. 각 상태는 EntityManager의 메서드를 통해 전환되며, 영속성 컨텍스트의 관리 여부와 데이터베이스 동기화 상태에 따라 결정된다. 영속성 컨텍스트는 1차 캐시, 변경 감지, 쓰기 지연 같은 기능을 제공해 개발 생산성을 높이고 성능을 최적화한다. 또한 flush와 clear의 동작 시점을 이해하면 메모리 사용량도 효과적으로 관리할 수 있다. 결국 엔티티 상태를 정확히 이해하는 것은 LazyInitializationException 같은 예외를 줄이고, 트랜잭션 범위 내에서 엔티티를 안전하게 다루기 위한 기본 지식이다.
