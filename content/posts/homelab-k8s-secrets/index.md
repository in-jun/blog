...
Vault 설치 후에는 초기화(initialization)와 언실링(unsealing)이라는 두 가지 중요한 단계가 필요하다. 이 과정에서는 암호화 키를 생성하고 Vault를 사용할 수 있는 상태로 전환하며, 보안을 위해 자동화하지 않고 수동으로 진행한다.
...
> **Shamir의 비밀 공유 알고리즘**
>
> Vault는 Shamir의 비밀 공유(Shamir's Secret Sharing) 알고리즘을 사용하여 마스터 키를 여러 조각으로 분할하며, 기업 환경에서는 이 5개의 키를 서로 다른 관리자에게 분산시켜 최소 3명이 동의해야만 Vault를 열 수 있도록 상호 검증 원칙을 적용한다. 홈랩에서는 한 사람이 관리하지만 엔터프라이즈 보안 원칙을 경험해볼 수 있다.
...
쿠버네티스 인증은 파드가 서비스 계정 토큰으로 Vault에 붙도록 만들기 위해 설정했다. 실제로는 Vault 파드 안에서 아래 명령을 실행했다:
...
두 번째로는 ArgoCD Vault Plugin도 붙여봤다. 이 방식은 Git 저장소에는 시크릿 참조만 남기고, 실제 값은 ArgoCD가 배포 시점에 Vault에서 가져오게 한다.
