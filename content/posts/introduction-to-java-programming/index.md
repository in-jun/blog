...
Java는 1995년 Sun Microsystems에서 James Gosling이 이끄는 팀에 의해 개발된 객체지향 프로그래밍 언어이다. "Write Once, Run Anywhere(한 번 작성하면 어디서나 실행)"라는 슬로건 아래 플랫폼 독립적인 실행 환경을 제공하며, 2024년 현재 TIOBE 프로그래밍 언어 순위에서 꾸준히 상위권을 유지하고 있다. 기업용 애플리케이션, 안드로이드 앱, 빅데이터 처리, 웹 서비스 등 다양한 분야에서 널리 사용되며, 강력한 타입 시스템과 풍부한 표준 라이브러리, 활발한 커뮤니티를 바탕으로 지속적으로 발전하고 있다.
...
Java의 역사는 1991년 Sun Microsystems의 "Green Project"에서 출발했다. James Gosling, Mike Sheridan, Patrick Naughton이 가전제품용 프로그래밍 언어를 개발하기 위해 추진한 이 프로젝트가 Java의 기원이 되었다.
...
| **사용 목적** | IS-A 관계, 공통 기능 | 구현해야 할 기능의 계약 정의 |
...
Java는 1995년 탄생 이후 30년 가까이 발전해 온 객체지향 프로그래밍 언어다. JVM 기반의 플랫폼 독립성, 강력한 타입 시스템, 자동 메모리 관리, 풍부한 표준 라이브러리를 바탕으로 기업용 애플리케이션 개발의 핵심 언어로 자리잡았다.

캡슐화, 상속, 다형성, 추상화 같은 객체지향의 핵심 원칙을 이해하면 유지보수가 쉽고 확장 가능한 코드를 작성할 수 있다. 여기에 제네릭과 컬렉션 프레임워크를 함께 익히면 타입 안전성과 데이터 구조 활용 능력을 한층 높일 수 있으며, Java는 람다 표현식, 스트림 API, 가상 스레드 같은 기능을 통해 현대적인 프로그래밍 패러다임도 계속 수용하고 있다.
