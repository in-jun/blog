...
웹 인증(Web Authentication)은 HTTP 프로토콜의 무상태(Stateless) 특성 때문에 발생하는 사용자 식별 문제를 해결하기 위한 핵심 메커니즘이다. 1994년 Netscape Communications의 Lou Montulli가 쿠키를 발명한 이후 인증 방식은 세션 기반 인증과 토큰 기반 인증으로 발전해왔고, 오늘날에는 보안성과 확장성을 함께 고려해 JWT와 Refresh Token을 조합한 하이브리드 방식이 널리 사용된다.
...
> **Cookie란?**
>
> 쿠키(Cookie)는 서버가 클라이언트(브라우저)에 전송해 저장하는 작은 텍스트 데이터다. 1994년 Netscape의 Lou Montulli가 웹 쇼핑 카트를 구현하기 위해 고안했으며, 최대 4KB까지 저장할 수 있다. 쿠키는 이후 같은 서버로 요청을 보낼 때 자동으로 함께 전송되어 상태 정보를 유지하는 데 사용된다.
...
> **Session이란?**
>
> 세션(Session)은 서버가 클라이언트의 상태 정보를 직접 관리하는 방식이다. 클라이언트에는 고유한 세션 ID만 전달하고, 실제 사용자 정보는 서버의 메모리나 데이터베이스에 저장한다. 이런 구조 덕분에 쿠키만 사용하는 방식보다 보안성이 높아졌고, 1990년대 후반 PHP, ASP 같은 서버 사이드 환경에서 표준적인 인증 방식으로 널리 사용되었다.
...
> **JWT란?**
>
> JWT(JSON Web Token)는 JSON 객체를 Base64Url로 인코딩해 정보를 주고받는 토큰 형식이며, 2010년 IETF(Internet Engineering Task Force)에서 RFC 7519로 표준화되었다. 필요한 정보를 토큰 자체에 담을 수 있어 서버에 상태를 저장하지 않는 무상태(Stateless) 인증에 적합하며, 마이크로서비스 아키텍처와 분산 시스템에서 널리 사용된다.
...
**Header**는 토큰 타입(`typ`)과 서명 알고리즘(`alg`)을 포함하고, **Payload**는 사용자 정보와 토큰 메타데이터(발급 시간 `iat`, 만료 시간 `exp`, 발급자 `iss` 등)를 담는다. **Signature**는 Header와 Payload에 서명용 키를 적용해 생성하며, 토큰의 무결성을 검증하는 데 사용된다.
...
> **Refresh Token이란?**
>
> Refresh Token은 수명이 짧은 Access Token의 한계를 보완하기 위해 사용하는 장기 유효 토큰이다. Access Token이 만료되면 사용자는 다시 로그인하지 않고도 새로운 Access Token을 발급받을 수 있다. OAuth 2.0 스펙(RFC 6749)에서도 공식적으로 정의되어 있으며, 세션 방식의 보안성과 JWT 방식의 확장성을 결합한 하이브리드 인증의 핵심 요소로 쓰인다.
...
>
> Refresh Token Rotation은 Access Token을 재발급할 때마다 새로운 Refresh Token도 함께 발급하고, 이전 Refresh Token은 즉시 무효화하는 보안 강화 전략이다. OAuth 2.0 Security Best Current Practice(RFC 6819)에서 권장하는 방식이며, Refresh Token 탈취로 인한 장기적인 보안 위협을 줄이고 토큰 재사용 공격(Token Replay Attack)을 탐지하는 데 도움이 된다.
...
웹 인증 방식은 Cookie, Session, JWT, Refresh Token, RTR로 발전해왔고, 각 방식은 저마다 장단점을 가진다. Cookie는 클라이언트 측 상태 유지의 기초를 제공하고, Session은 서버 측 보안을 강화하며, JWT는 무상태 인증과 마이크로서비스 환경에서의 확장성을 제공한다. Refresh Token과 RTR은 JWT의 보안 취약점을 보완해 실무에서 많이 쓰이는 하이브리드 인증 방식을 완성한다.

인증 방식을 선택할 때는 서비스 특성, 보안 요구사항, 확장성, 구현 복잡도를 함께 고려해야 한다. 어떤 방식을 택하더라도 HTTPS 사용, 적절한 토큰 만료 시간 설정, 안전한 저장소 사용, 감사 로깅 같은 보안 베스트 프랙티스는 반드시 함께 적용해야 한다.
