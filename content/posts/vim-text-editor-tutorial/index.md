...
모달 에디팅은 키보드의 문자 키를 명령어로 활용할 수 있게 해, Ctrl이나 Alt 같은 수식 키 없이도 많은 작업을 간결하게 표현할 수 있다. 또한 텍스트 입력과 편집을 분리해 의도치 않은 입력을 줄이고, 명령어를 조합(composability)해 복잡한 편집 작업도 짧게 표현할 수 있다. 초기 학습 곡선은 가파르지만, 기본 명령어 20-30개만 익혀도 일상적인 편집은 충분히 가능하다.
...
| 명령어 | 동작 | 설명 |
|--------|------|------|
| `x` | 커서 위치 문자 삭제 | 현재 문자 삭제 |
| `r{문자}` | 커서 위치 문자 교체 | 현재 문자를 다른 문자로 바꾸기 |
| `dd` | 현재 줄 삭제 | 현재 줄 삭제하기 |
| `yy` | 현재 줄 복사 | 현재 줄 복사하기 |
| `p` | 커서 뒤에 붙여넣기 | 뒤쪽에 붙여넣기 |
| `P` | 커서 앞에 붙여넣기 | 앞쪽에 붙여넣기 |
| `u` | 실행 취소 | 마지막 변경 되돌리기 |
| `Ctrl+r` | 다시 실행 | 되돌린 변경 다시 적용 |
...
| 조합 | 의미 | 설명 |
|------|------|------|
| `dw` | 단어 삭제 | 커서부터 단어 끝까지 삭제 |
| `d$` | 끝까지 삭제 | 커서부터 줄 끝까지 삭제 |
| `d0` | 시작까지 삭제 | 줄 시작까지 삭제 |
| `diw` | 현재 단어 내부 삭제 | 단어 전체 삭제 |
| `daw` | 단어 하나 삭제 | 단어와 주변 공백 삭제 |
| `ci"` | 따옴표 내부 변경 | 따옴표 안 내용 변경 |
| `ca(` | 괄호 포함 변경 | 괄호 포함 내용 변경 |
| `yiw` | 현재 단어 복사 | 단어 복사 |
| `>}` | 문단 끝까지 들여쓰기 | 문단까지 들여쓰기 |
...
| 명령어 | 동작 | 설명 |
|--------|------|------|
| `i` | 커서 위치에서 입력 | 현재 위치에서 입력 시작 |
| `a` | 커서 다음 위치에서 입력 | 다음 위치에서 입력 시작 |
| `I` | 줄 맨 앞에서 입력 | 줄의 첫 위치에서 입력 시작 |
| `A` | 줄 맨 뒤에서 입력 | 줄의 끝에서 입력 시작 |
| `o` | 아래에 새 줄 만들고 입력 | 아래쪽에 새 줄을 열고 입력 |
| `O` | 위에 새 줄 만들고 입력 | 위쪽에 새 줄을 열고 입력 |
| `s` | 문자 삭제 후 입력 | 현재 문자를 바꾸며 입력 |
| `S` | 줄 삭제 후 입력 | 현재 줄을 바꾸며 입력 |
| `c{이동}` | 범위 삭제 후 입력 | 지정한 범위를 바꾸기 |
...
매크로는 반복 작업을 자동화할 때 유용한 기능이다. `q{레지스터}`로 녹화를 시작하고 작업을 수행한 뒤 `q`로 종료한다. 이후 `@{레지스터}`로 재생할 수 있고, `@@`로 마지막 매크로를 반복할 수 있다.
...
vimtutor는 Vim에 내장된 대화형 튜토리얼로, 터미널에서 `vimtutor` 명령어를 입력하면 실행된다. 전체 분량은 약 30분 정도이며, 한글 버전도 `vimtutor ko`로 실행할 수 있다. 기본 명령어를 직접 실습하면서 익힐 수 있어 초보자가 시작하기에 적합하다. 매일 조금씩 사용하면서 새로운 명령어를 하나씩 익히는 방식이 효과적이며, Vim 키 바인딩은 VS Code(Vim 확장), JetBrains IDE(IdeaVim), 브라우저(Vimium) 같은 다양한 도구에서도 활용할 수 있다.

## 결론

Vim은 오래된 역사를 가진 텍스트 편집기이지만, 지금도 서버 환경과 터미널 작업에서 여전히 실용적이다. 모달 에디팅에 익숙해지기까지는 시간이 필요하지만, 기본 이동과 편집 명령만 익혀도 효율적인 작업이 가능하다. 처음에는 `vimtutor`로 기초를 다지고, 자주 쓰는 명령어부터 차근차근 늘려 가면 된다.
