...
이 주소는 로컬 네트워크 세그먼트 안에서만 전달되고 라우터를 넘지 않는 제한된 브로드캐스트 주소다. 예를 들어 DHCP 클라이언트가 아직 IP 주소를 할당받지 못한 상태에서 DHCP 서버를 찾을 때, 목적지 주소로 255.255.255.255를 사용해 같은 물리적 네트워크의 모든 호스트에게 DHCP Discover 메시지를 보낸다.
