...
tags: ["HTTP", "프로토콜", "웹"]
draft: false
description: "HTTP 상태 코드의 분류와 각 코드의 의미를 설명한다."
---

HTTP 상태 코드(HTTP Status Code)는 클라이언트가 서버에 요청을 보낸 뒤, 서버가 그 처리 결과를 알려주기 위해 반환하는 세 자리 숫자의 표준 응답 코드다. 이 코드는 웹 브라우저, API 클라이언트, 검색 엔진 등 모든 HTTP 기반 통신에서 요청의 성공 여부, 리다이렉션 필요성, 클라이언트나 서버 측 오류 발생 여부를 명확하게 전달한다. RESTful API를 설계할 때 적절한 상태 코드를 선택하는 일은 API의 직관성과 개발자 경험을 크게 좌우한다.
...
3xx 상태 코드는 클라이언트가 요청을 완료하려면 추가 작업이 필요함을 나타낸다. 주로 URL 변경, 캐싱 검증, 리소스 이동 시 사용되며, 사용자를 올바른 위치로 안내하거나 네트워크 효율성을 높이는 역할을 한다.

> **리다이렉션의 종류**
>
> 리다이렉션은 크게 영구 리다이렉션(301, 308)과 임시 리다이렉션(302, 303, 307)으로 나뉜다. 영구 리다이렉션은 검색 엔진이 새 URL을 색인하고 기존 평가 신호를 새 주소에 반영하는 반면, 임시 리다이렉션은 원래 URL을 유지한다.
...
**301 Moved Permanently**는 URL이 영구적으로 변경되었음을 나타낸다. 다만 일부 클라이언트는 이 응답을 받으면 POST 요청을 GET으로 바꾸기도 하는데, 이는 HTTP/1.0 시절의 동작을 이어받은 결과다. 반면 **308 Permanent Redirect**는 HTTP 메서드를 반드시 유지하므로 POST 요청이 POST로 리다이렉트되어야 할 때 사용한다.
...
404 Not Found는 리소스가 존재하지 않을 때 사용하지만, 보안상 민감한 경우에는 403 대신 404를 반환해 리소스의 존재 여부를 숨길 수 있다. 예를 들어 다른 사용자의 비공개 리소스에 접근할 때 403을 반환하면 해당 리소스가 존재한다는 정보가 노출되므로, 404를 반환하는 편이 더 안전할 수 있다.
...
409 Conflict는 리소스의 현재 상태와 충돌이 발생했을 때 사용하며, 버전 충돌(낙관적 잠금 실패), 중복 리소스 생성 시도, 삭제된 리소스를 수정하려는 시도 등의 상황에서 반환한다.
...
HTTP 상태 코드는 클라이언트와 서버 간 통신의 핵심 요소로, 1991년 HTTP/0.9의 단순한 형태에서 현재의 정교한 체계로 발전해왔다. 올바른 상태 코드를 선택하면 API의 직관성, 디버깅 용이성, 사용자 경험을 모두 개선할 수 있다. RESTful API를 설계할 때는 각 상황에 맞는 상태 코드를 고르고, 일관된 에러 응답 구조를 제공하며, 효과적인 모니터링으로 문제를 조기에 발견하고 대응해야 한다.
