...
**자동 반영(Pulled Automatically)**: 승인된 변경 사항은 소프트웨어 에이전트에 의해 자동으로 시스템에 적용되어야 하며, 사람이 수동으로 kubectl apply와 같은 명령어를 실행하는 것이 아니라 자동화된 프로세스가 지속적으로 Git 상태를 시스템에 반영해야 한다.
...
CI 파이프라인이 Config Repository의 Kubernetes 매니페스트에서 이미지 태그를 업데이트해 커밋하거나, 이미지 태그 자동 업데이트 기능(ArgoCD Image Updater, Flux Image Automation 등)을 활용해 새 이미지를 감지하고 매니페스트를 자동으로 업데이트할 수 있다.
...
Flux는 Weaveworks에서 개발한 GitOps 도구로, GitOps라는 용어를 처음 만든 회사에서 직접 개발한 도구이다. 현재 CNCF Graduated 프로젝트로 선정되어 ArgoCD와 함께 대표적인 GitOps 도구로 자리 잡고 있다.
...
GitOps는 2017년 Weaveworks에서 탄생한 이후 클라우드 네이티브 환경의 배포 자동화를 이끄는 핵심 패러다임으로 자리 잡았다. Push 방식은 기존 CI/CD 도구를 활용할 수 있어 도입이 쉽지만 보안성과 상태 일관성 측면에서 제약이 있고, Pull 방식은 보안성과 Self-Healing 기능이 뛰어나지만 전용 도구 도입과 학습이 필요하다. ArgoCD는 강력한 UI와 멀티 클러스터 지원을 바탕으로 대규모 팀에 적합하고, Flux는 경량의 CLI 중심 워크플로우를 선호하는 팀에 잘 맞는다. 결국 조직의 규모, 기술 역량, 보안 요구사항, 기존 인프라를 종합적으로 고려해 적절한 전략과 도구를 선택하되, 어떤 방식을 택하든 Git을 단일 진실 공급원으로 활용해 일관되고 추적 가능한 배포 프로세스를 구축하는 것이 GitOps의 핵심 가치이다.
