...
Docker는 컨테이너 기반 애플리케이션을 개발, 배포, 실행하기 위한 플랫폼이다. 리눅스 커널의 네임스페이스와 cgroups를 활용해 격리된 환경을 만들며, 이 가이드에서는 우분투 리눅스에 Docker를 설치하고 첫 컨테이너를 실행하는 과정을 단계별로 설명한다.
...
새로 설치하기 전에 시스템에 남아 있을 수 있는 비공식 Docker 패키지나 이전 버전을 먼저 제거한다. 이렇게 하면 패키지 충돌을 줄이고 깔끔한 설치 환경을 유지할 수 있다.
...
Docker 패키지의 무결성과 출처를 확인하려면 공식 GPG 키를 시스템에 추가해야 한다. 이 키로 패키지가 Docker에서 서명한 것인지 검증할 수 있다.
...
> **Docker 그룹 권한**
>
> 기본적으로 Docker 데몬은 root 권한으로 실행되므로 `docker` 명령어에도 `sudo`가 필요하다. 현재 사용자를 `docker` 그룹에 추가하면 `sudo` 없이 Docker 명령어를 실행할 수 있다.
...
실용적인 예제로 Nginx 웹 서버 컨테이너를 실행한다.
...
> **Docker 네트워크란?**
>
> Docker 네트워크는 컨테이너 간 통신을 관리하는 가상 네트워크 인프라로, 컨테이너를 격리하거나 연결해 마이크로서비스 아키텍처를 구현할 수 있다.
...
> **Docker 볼륨이란?**
>
> Docker 볼륨은 컨테이너 데이터를 영구적으로 저장하는 방식이다. 컨테이너를 삭제해도 데이터가 남고, 여러 컨테이너가 같은 데이터를 공유할 수도 있다.
...
Docker 기본 설치와 사용법을 익혔다면 다음 주제로 범위를 넓혀 보면 좋다.
...
Docker 설치는 컨테이너 기술을 시작하는 첫 단계다. 공식 저장소를 사용하면 비교적 안정적으로 최신 버전을 유지할 수 있다. 여기에 기본적인 컨테이너 실행, 이미지 관리, 네트워크, 볼륨 구성을 익혀 두면 이후 Docker Compose나 Kubernetes 같은 주제로도 자연스럽게 이어갈 수 있다.
