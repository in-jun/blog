...
Docker 이미지 레이어(Layer)는 Docker 이미지를 구성하는 기본 단위다. 각 레이어는 파일 시스템의 변경 사항을 캡처하는 읽기 전용 계층으로, 이전 레이어 위에 차곡차곡 쌓인다. 이렇게 쌓인 레이어는 Union File System을 통해 하나의 파일 시스템으로 통합되어 컨테이너에 제공되며, 이 구조를 이해하는 일은 효율적인 이미지 빌드와 스토리지 최적화의 핵심이다.
...
여러 레이어가 스택처럼 쌓이면 파일 시스템은 몇 가지 규칙에 따라 동작한다. 상위 레이어의 파일은 하위 레이어의 동일 경로 파일을 가리며(shadow), 파일 삭제는 whiteout 파일로 표시된다. 파일을 읽을 때는 상위 레이어부터 순차적으로 검색해 처음 발견된 파일을 반환한다.
...
1. **파일 수정 요청**: 컨테이너가 `/etc/nginx/nginx.conf` 파일 수정을 시도
...
Copy-on-Write 메커니즘은 공간 효율성이 높고 컨테이너를 빠르게 시작할 수 있다는 장점이 있다. 다만 대용량 파일을 처음 수정할 때는 복사 오버헤드가 발생하고, 쓰기 집중 워크로드에서는 성능이 떨어질 수 있다. 이런 경우에는 볼륨 마운트를 사용해 CoW 오버헤드를 우회하는 것이 권장된다.
...
Docker는 다음 조건 중 하나라도 만족하면 해당 레이어와 그 이후 레이어의 캐시를 무효화한다. 따라서 레이어 순서는 빌드 성능에 직접적인 영향을 준다.
...
> **멀티 스테이지 빌드란?**
>
> 멀티 스테이지 빌드는 하나의 Dockerfile에서 여러 개의 FROM 명령어를 사용해 빌드 환경과 실행 환경을 분리하는 기법이다. 이를 통해 빌드 도구와 중간 결과물을 최종 이미지에서 제외해 이미지 크기를 크게 줄일 수 있다.

### 멀티 스테이지 빌드 예시

다음은 빌드 환경과 실행 환경을 분리해 이미지 크기를 줄이는 Go 애플리케이션 예시다.
...
이 예시에서 빌드 스테이지의 golang:1.21-alpine 이미지는 약 300MB이지만, 최종 실행 이미지는 alpine:3.19 기반으로 약 10MB가 되어 이미지 크기가 97% 이상 감소한다.
...
Docker 이미지 레이어 시스템은 효율적인 스토리지 사용, 빠른 이미지 빌드, 신속한 컨테이너 시작을 가능하게 하는 핵심 기술이다. 이 구조는 Union File System과 Copy-on-Write 메커니즘을 기반으로 동작하며, 이를 이해하면 빌드 시간 단축과 이미지 크기 감소에 직접 도움이 된다.

효과적인 Dockerfile을 작성하려면 변경 빈도가 낮은 레이어를 먼저 배치하고, 관련 명령어는 가능한 한 통합하는 편이 좋다. 여기에 멀티 스테이지 빌드와 `docker history`, `dive` 같은 분석 도구를 함께 활용하면 최적화 지점을 더 쉽게 찾을 수 있다.
