...
인터넷에서 웹사이트에 접속할 때 우리는 IP 주소 대신 도메인 이름을 사용한다. 이 도메인 이름을 실제 서버의 IP 주소로 변환하는 시스템이 바로 DNS(Domain Name System)다. DNS는 흔히 인터넷의 전화번호부에 비유되며, 1983년 Paul Mockapetris가 설계한 이후 인터넷의 핵심 인프라로 자리잡았다. 오늘날 DNS는 매일 수십억 건의 쿼리를 처리하며, 사용자가 192.168.0.1이나 2001:4860:4860::8888 같은 복잡한 IP 주소를 외우지 않고도 www.example.com처럼 직관적인 도메인 이름으로 웹 서비스에 접근할 수 있게 한다.

> **DNS(Domain Name System)란?**
>
> DNS는 사람이 읽기 쉬운 도메인 이름(예: www.example.com)을 컴퓨터가 이해할 수 있는 IP 주소(예: 93.184.216.34)로 변환하는 분산형 데이터베이스 시스템이다. 전 세계에 분산된 계층형 네임서버들이 협력해 이름 해석 서비스를 제공하며, 관련 표준은 IETF의 RFC 1034와 RFC 1035에 정의되어 있다.

## DNS의 탄생 배경

인터넷 초기인 1970년대에는 네트워크에 연결된 호스트 수가 수백 대에 불과했다. 당시에는 Stanford Research Institute(SRI)에서 관리하는 단일 텍스트 파일인 HOSTS.TXT를 사용해 호스트 이름과 IP 주소를 매핑했고, 네트워크 관리자들은 FTP로 이 파일을 주기적으로 내려받아 자신의 시스템(`/etc/hosts`)에 적용했다. HOSTS.TXT는 단순하고 이해하기 쉬운 방식이었지만, ARPANET이 성장하면서 호스트 수가 급격히 늘어나자 중앙 집중식 파일 관리 방식은 확장성(파일 크기 증가), 일관성(업데이트 충돌), 트래픽(다운로드 부하) 측면에서 한계를 드러냈다.

1983년 Paul Mockapetris는 이러한 문제를 해결하기 위해 분산형, 계층형 이름 해석 시스템인 DNS를 설계했다. DNS의 개념과 구현은 RFC 882와 RFC 883에서 처음 정의되었고, 이후 1987년에 RFC 1034와 RFC 1035로 정리되었다. DNS의 핵심 설계 목표는 각 조직이 자신의 도메인을 독립적으로 관리할 수 있는 분산된 권한 구조, 수백만 개의 도메인을 수용할 수 있는 확장 가능한 계층 구조, 반복 쿼리를 줄이는 효율적인 캐싱을 바탕으로 대규모 인터넷 환경에서도 안정적인 이름 해석 서비스를 제공하는 것이었다. 이러한 설계 철학은 40년이 지난 지금도 여전히 유효하다.
...
DNS 계층 구조의 최상위에 위치하는 루트 도메인은 빈 문자열로 표현되며, FQDN(Fully Qualified Domain Name)에서는 마지막 점으로 표시된다(예: www.example.com.). 이를 관리하는 것은 전 세계에 분산된 13개의 논리적 루트 네임서버 클러스터(A-root부터 M-root까지, 각각 a.root-servers.net부터 m.root-servers.net)다. 실제 운영은 Anycast 기술을 기반으로 전 세계에 배치된 수백 개의 서버 인스턴스가 맡고 있어, 사용자는 가장 가까운 루트 서버에 자동으로 연결된다. 이 구조 덕분에 높은 가용성(99.99% 이상)과 낮은 지연 시간을 유지할 수 있다. 루트 네임서버는 ICANN(Internet Corporation for Assigned Names and Numbers), Verisign, NASA, US Army, University of Maryland 등 여러 기관이 협력해 운영한다.
...
재귀적 리졸버는 클라이언트의 DNS 쿼리를 받아 최종 IP 주소를 찾아 반환하는 서버다. ISP가 제공하는 DNS 서버나 Google Public DNS(8.8.8.8, 8.8.4.4), Cloudflare DNS(1.1.1.1, 1.0.0.1), OpenDNS(208.67.222.222, 208.67.220.220), Quad9(9.9.9.9) 같은 공용 DNS 서버가 이에 해당한다. 리졸버는 응답을 캐싱해 반복적인 쿼리를 줄이고 응답 속도를 높이며(일반적으로 5~50ms 이내), 일부 서비스는 DNSSEC 검증(응답 무결성 확인), 악성 도메인 차단(멀웨어, 피싱 사이트 필터링), 쿼리 로깅, 프라이버시 보호(쿼리 암호화, 로그 비저장) 같은 부가 기능도 제공한다.
...
권한 네임서버는 특정 도메인의 DNS 레코드를 실제로 저장하고 관리하는 서버로, 도메인 소유자가 직접 운영(BIND, PowerDNS, NSD 등 소프트웨어 사용)하거나 DNS 호스팅 서비스(AWS Route 53, Cloudflare DNS, Google Cloud DNS, Azure DNS 등)를 통해 관리된다. 권한 네임서버만이 해당 도메인의 DNS 정보에 대한 공식 원본이며, 다른 서버들은 이 정보를 캐싱해 제공한다. 일반적으로 2개 이상의 권한 네임서버를 운영해(Primary, Secondary) 중복성과 장애 복구를 보장한다.
...
SOA(Start of Authority) 레코드는 DNS 존(Zone)의 권한 정보와 동작 파라미터를 정의한다. 여기에는 주 네임서버(Primary Name Server), 관리자 이메일(@ 대신 .로 표현), 존 일련번호(Serial Number, 존 파일 변경 시 증가), 새로 고침 간격(Refresh), 재시도 간격(Retry), 만료 시간(Expire), 최소 TTL(Negative TTL) 등의 정보가 포함된다. 이 값들은 2차 서버의 동기화 주기, 연결 실패 시 재시도 방식, 존재하지 않는 레코드의 캐싱 기간 등에 영향을 준다. SOA 레코드는 존마다 하나만 존재해야 하며(RFC 1035), 존 전송(Zone Transfer, AXFR/IXFR)과 캐싱 동작에도 영향을 미친다.
...
이번 글에서는 DNS의 탄생 배경부터 계층 구조, 동작 원리, 다양한 레코드 유형, 캐싱 메커니즘, 보안 기능까지 도메인 네임 시스템의 전반을 살펴보았다. DNS는 1983년 설계된 이후 40년이 넘는 시간 동안 인터넷의 핵심 인프라로 자리해 왔으며, 사용자가 복잡한 IP 주소 대신 도메인 이름으로 서비스에 접근할 수 있게 해왔다. 또한 분산된 계층 구조와 효율적인 캐싱을 바탕으로 막대한 쿼리를 안정적으로 처리해 왔고, 최근에는 DNSSEC, DoH, DoT 같은 기술을 통해 보안과 프라이버시 측면에서도 계속 발전하고 있다.
