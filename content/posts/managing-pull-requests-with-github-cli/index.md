...
GitHub CLI(gh)는 GitHub에서 2020년 9월에 정식 출시한 공식 명령줄 인터페이스 도구다. 터미널에서 Pull Request 생성, 이슈 관리, 저장소 관리 같은 핵심 작업을 처리할 수 있어, 웹 브라우저와 터미널을 오가던 흐름을 한곳으로 모아준다. 개발자들이 코드 작성과 버전 관리를 주로 터미널에서 수행한다는 점을 고려하면, GitHub CLI는 컨텍스트 스위칭을 줄이고 반복 작업을 자동화하는 데 특히 유용하다.
...
GitHub CLI는 기존의 `hub` 명령어를 대체하기 위해 개발되었다. GitHub에서 직접 개발하고 유지보수하기 때문에 새로운 기능을 비교적 빠르게 지원하며, Go 언어로 작성되어 다양한 플랫폼에서 단일 바이너리로 실행할 수 있다. 또한 오픈소스로 공개되어 커뮤니티의 기여도 활발하게 이루어지고 있다.

### 주요 이점

**효율성 향상**: 마우스와 웹 브라우저를 오가지 않고 키보드만으로 GitHub 작업을 처리할 수 있다. IDE나 터미널에서 바로 PR을 생성하거나 리뷰할 수 있어 개발 흐름이 끊기지 않는다.

**자동화 지원**: 스크립트와 CI/CD 파이프라인에 GitHub 작업을 통합할 수 있다. 특정 조건에서 PR 생성, 라벨 추가, 리뷰어 지정 같은 작업을 자동화할 수 있고, JSON 출력을 지원해 다른 도구와 연동하기도 쉽다.

**일관된 인터페이스**: macOS, Linux, Windows 등 주요 플랫폼에서 동일한 명령어를 사용할 수 있다. 팀원 간 워크플로우를 공유하고 문서화하기 쉽고, 새로운 환경에서도 같은 방식으로 작업할 수 있다.

**풍부한 기능**: PR과 Issue 관리뿐 아니라 저장소 생성 및 복제, GitHub Actions 관리, Gist 생성, 릴리스 관리, Codespaces 접속까지 폭넓게 지원한다.
...
GitHub CLI를 사용하려면 먼저 GitHub 계정으로 인증해야 한다. 인증 방식은 브라우저 기반 OAuth 인증과 Personal Access Token 인증 두 가지를 지원하며, 대부분의 경우 브라우저 기반 인증이 더 간편하고 안전하다.
...
저장소에 `.github/PULL_REQUEST_TEMPLATE.md` 파일을 두면 PR 생성 시 해당 내용이 PR 본문에 자동으로 채워진다. 여러 템플릿을 쓰고 싶다면 `.github/PULL_REQUEST_TEMPLATE/` 디렉토리에 마크다운 파일을 여러 개 두고 `--template` 옵션으로 선택하면 된다.
...
GitHub CLI는 터미널 중심의 개발 흐름에서 GitHub 작업을 더 자연스럽게 이어 주는 도구다. PR 생성, 리뷰, 병합 같은 일상적인 작업을 한곳에서 처리할 수 있고, 자동화와 스크립팅에도 잘 어울린다. 키보드 중심의 워크플로우를 선호한다면 개인 생산성을 높이는 데 도움이 되고, 팀 차원에서도 일관된 PR 절차를 유지하기 쉽다.
