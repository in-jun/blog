...
디스패처 서블릿은 스프링 MVC의 핵심 컴포넌트다. 클라이언트의 모든 HTTP 요청을 단일 진입점에서 받아 적절한 컨트롤러로 전달하고, 컨트롤러가 반환한 결과를 View로 렌더링해 응답하는 Front Controller 패턴의 구현체이기도 하다. 모든 요청을 중앙에서 처리하므로 인증, 로깅, 예외 처리 같은 공통 로직을 효율적으로 관리할 수 있고, 개발자는 비즈니스 로직에 더 집중할 수 있다.
...
ViewResolver는 컨트롤러가 반환한 논리적 뷰 이름을 실제 뷰 객체로 변환하는 역할을 한다. 예를 들어 `InternalResourceViewResolver`는 JSP 파일 경로로, `ThymeleafViewResolver`는 Thymeleaf 템플릿으로 변환한다. 이때 설정된 prefix와 suffix를 조합해 실제 뷰 파일의 경로를 만든다.
...
`@Controller`는 전통적인 MVC 패턴을 따라 ModelAndView를 반환하고, ViewResolver를 통해 HTML 페이지를 렌더링한다. 반면 `@RestController`는 `@Controller`와 `@ResponseBody`를 결합한 어노테이션으로, 메서드의 반환 값이 뷰 이름이 아니라 HTTP 응답 본문으로 직접 전달된다. 이때 `HttpMessageConverter`가 자바 객체를 JSON, XML 등의 포맷으로 직렬화한다. 가장 널리 쓰이는 `MappingJackson2HttpMessageConverter`는 Jackson으로 객체를 JSON으로 변환하고, `GsonHttpMessageConverter`는 Gson을 사용한다. 적절한 컨버터는 `Accept` 헤더와 `Content-Type` 헤더를 기준으로 자동 선택된다.
...
Spring Boot는 `DispatcherServletAutoConfiguration`을 통해 DispatcherServlet을 자동으로 설정하고 등록한다. 덕분에 개발자는 별도의 web.xml이나 자바 설정 없이도 웹 애플리케이션을 바로 실행할 수 있다. `spring.mvc.*` 프로퍼티로도 여러 동작을 조정할 수 있다. 예를 들어 `spring.mvc.view.prefix`와 `spring.mvc.view.suffix`로 ViewResolver를 설정하고, `spring.mvc.static-path-pattern`으로 정적 리소스 경로를 지정하며, `spring.mvc.throw-exception-if-no-handler-found`로 핸들러를 찾지 못했을 때 예외를 던질지 제어할 수 있다. 더 세밀한 커스터마이징이 필요하면 `WebMvcConfigurer`를 구현해 인터셉터, 포매터, 메시지 컨버터, CORS 설정 등을 추가하거나 수정할 수 있다. 반대로 `@EnableWebMvc`를 사용하면 자동 설정을 비활성화하고 수동 설정으로 전환할 수 있다.

## 실전 팁

### Interceptor vs Filter 차이

Filter는 서블릿 스펙의 일부로, 서블릿 컨테이너 레벨에서 동작한다. 따라서 DispatcherServlet 실행 전후에 요청과 응답을 가로챌 수 있다. 반면 Interceptor는 스프링 MVC의 일부로 DispatcherServlet 내부에서 동작하며, HandlerMapping 이후부터 Handler 실행 전후의 흐름에 개입한다. 또 스프링 빈에 접근할 수 있고 ModelAndView도 수정할 수 있다. 일반적으로 Filter는 보안, 인코딩 같은 저수준 처리에 적합하고, Interceptor는 인증, 권한 검사처럼 스프링 MVC 흐름과 밀접한 처리에 적합하다. Interceptor는 `HandlerInterceptor` 인터페이스의 `preHandle()`, `postHandle()`, `afterCompletion()` 메서드를 구현해 각 단계에 개입한다.
...
디스패처 서블릿은 Spring Boot를 사용할 경우 기본적으로 자동 설정된다. 하지만 필요하면 수동으로도 설정할 수 있다. `@Configuration` 클래스에서 `DispatcherServlet`을 빈으로 등록하면 된다.
...
이렇게 설정하면 핸들러를 찾지 못했을 때 예외를 던지도록 만들 수 있다.

## 결론

DispatcherServlet은 Spring MVC에서 모든 HTTP 요청을 중앙에서 처리하는 핵심 컴포넌트다. Front Controller 패턴을 바탕으로 HandlerMapping, HandlerAdapter, ViewResolver 같은 전략 인터페이스를 조합해 유연하고 확장 가능한 웹 계층을 만든다. 또한 전통적인 MVC와 RESTful API 개발을 모두 지원하며, 인터셉터, 예외 처리, 파일 업로드, 국제화 같은 기능도 일관된 방식으로 통합한다. 결국 DispatcherServlet은 웹 계층의 복잡성을 감추고, 개발자가 비즈니스 로직에 집중할 수 있도록 돕는 스프링 MVC의 중심이라고 볼 수 있다.
